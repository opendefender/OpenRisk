  ...
  return (
    <PageFrame wide>
      <PageHeader
        ...
        count={tr(
          'Piste d’audit · Approbations · Délégations',
          'Audit trail · Approvals · Delegations',
        )}
      />
...
function AuditView({ isAdmin }: { isAdmin: boolean }) {
    ...
    () => [
      {
        ...
        render: (e) => (
          <span
            ...
            style={{ color: 'var(--fg-inverse)', background: ACTION_COLOR[e.action] }}
          >
            {e.action}
          </span>
        ),
      },
      {
        ...
        render: (e) => (
          <span className="text-[13px] block max-w-[420px] truncate">
      ...
      {
        key: 'entity',
        header: tr('Entité', 'Entity'),
        exportValue: (e) => e.entity_type,
        render: (e) => (
          <span className="text-[12px] mono" style={{ color: 'var(--fg-secondary)' }}>
            {e.entity_type}
          </span>
        ),
      },
      ...
      {
        key: 'when',
        header: tr('Quand', 'When'),
        exportValue: (e) => e.created_at,
        render: (e) => (
          <span className="text-[12px] mono" style={{ color: 'var(--fg-secondary)' }}>
            {fmt(e.created_at)}
          </span>
  ...
  const rowActions: RowAction<AuditEvent>[] = useMemo(
    () => [
      {
        key: 'diff',
        label: tr('Voir le détail', 'View details'),
        icon: FileClock,
  ...
  return (
    <>
      ...
      <DataTable
        ...
        toolbarExtra={
          <Btn
            label={exporting ? tr('Export…', 'Exporting…') : tr('Exporter CSV', 'Export CSV')}
            icon={Download}
            onClick={exportCsv}
            disabled={exporting}
          />
        ...
        empty={
          <EmptyState
            ...
            description={tr(
              'Chaque création, modification et suppression d’une entité auditée est enregistrée ici — qui, quoi, quand, et le détail Avant → Après. Elle se remplit dès votre première action.',
              'Every create, update and delete on an audited entity is recorded here — who, what, when, and the before/after diff. It fills up with your first action.',
            )}
          />
        }
      />
...
function AuditDetailDrawer({ e, onClose }: { e: AuditEvent; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="px-[22px] pt-5 pb-3.5 flex items-start gap-3"
        ...
        <div className="flex-1 overflow-y-auto px-[22px] py-5 text-[12.5px]">
          {changed.length > 0 ? (
            <div className="space-y-2">
              {changed.map((f) => (
                <div key={f} className="flex items-start gap-2 flex-wrap">
                  <span className="mono font-semibold" style={{ minWidth: 140 }}>
                    {f}
                  </span>
                  <span
                  ...
                  >
                    {renderVal(e.before?.[f])}
                  </span>
                  <ArrowRight size={12} style={{ marginTop: 3, color: 'var(--fg-secondary)' }} />
                  <span
                    className="mono px-1.5 rounded"
                    style={{ background: 'color-mix(in srgb, var(--low) 14%, transparent)' }}
                  >
                    {renderVal(e.after?.[f])}
                  </span>
                </div>
            ...
            <pre
              className="text-[11.5px] mono overflow-x-auto p-2 rounded"
              style={{ background: 'var(--bg-hover)' }}
            >
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex-1" />
        <Btn
          label={tr('Demander une approbation', 'Request approval')}
          icon={Plus}
          primary
      ...
      {isLoading && requests.length === 0 ? (
        ...
        <EmptyState
          ...
          description={tr(
            'Les demandes soumises via un workflow Maker-Checker atterrissent ici pour validation. Vous ne pouvez jamais approuver vos propres demandes.',
            'Requests submitted through a Maker-Checker workflow land here for sign-off. You can never approve your own request.',
          )}
        />
      ) : (
        <div className="space-y-3">
...
function ApprovalCard({ req }: { req: ApprovalRequest }) {
  ...
  const decide = async (decision: 'approve' | 'reject') => {
    ...
    if (decision === 'reject' && !comment.trim()) {
      toast.error(
        tr(
          'Un commentaire est obligatoire pour refuser — dites ce qui devrait changer.',
    ...
    try {
      ...
      toast.success(
        decision === 'approve'
          ? tr('Étape approuvée', 'Step approved')
          : tr('Demande rejetée', 'Request rejected'),
      );
      setComment('');
    } catch (err) {
  ...
  const cancel = async () => {
    try {
      await cancelApproval.mutateAsync(req.id);
      toast.success(tr('Annulée', 'Cancelled'));
    } catch {
    ...
    <Card style={{ padding: '14px 16px' }}>
      <div className="flex items-start gap-3 flex-wrap">
        <div className="flex-1 min-w-[240px]">
          ...
          <div className="text-[12px] mt-1" style={{ color: 'var(--fg-secondary)' }}>
            {req.workflow_name} ·{' '}
            {detail?.request_type_info
              ? lang === 'fr'
                ? detail.request_type_info.label
                : detail.request_type_info.label_en
          ...
          <div className="flex items-center gap-1.5 mt-2 flex-wrap">
            {(progress.length > 0
              ...
              : req.steps.map((st, i) => ({
                  ...
                  required_approvals: st.min_approvals,
                  approvals: 0,
                  satisfied: i < req.current_step,
                  rejected: false,
                  open: i === req.current_step && isPending,
                  approvers: [],
                }))
            ).map((p, i, arr) => (
        ...
        <div className="mt-3">
          ...
          {detail && (
            <p
              ...
              style={{ color: canDecide ? 'var(--fg-secondary)' : 'var(--medium)' }}
            >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {parallel && openSteps.length > 1 && (
              <select
                value={stepOrder ?? openSteps[0]?.order ?? 0}
        ...
        <div className="mt-2 space-y-1 pl-5">
          {req.decisions.map((d, i) => (
            <div key={i} className="text-[12px]" style={{ color: 'var(--fg-secondary)' }}>
              ...
              · {tr('étape', 'step')} {d.step_order + 1} ·{' '}
              {d.approver_email || d.approver_id.slice(0, 8)} · {fmt(d.decided_at)}
              {d.comment ? ` — “${d.comment}”` : ''}
            </div>
          ))}
        </div>
...
function SubmitModal({ onClose }: { onClose: () => void }) {
  ...
  const pick = (w: ApprovalWorkflow) => {
    setEntityType(w.entity_type);
    setAction(w.action);
  };

  const submit = async () => {
    if (!entityType || !title) {
      toast.error(tr('Type d’entité et titre requis', 'Entity type and title required'));
      return;
    }
    try {
  ...
  return (
    <ModalShell
      title={tr('Demander une approbation', 'Request approval')}
      onClose={onClose}
      onSubmit={submit}
      submitLabel={tr('Soumettre', 'Submit')}
    >
      {workflows.length > 0 && (
        <div>
          ...
          <div className="flex gap-1.5 flex-wrap mt-1">
            {workflows
              ...
              .map((w) => (
                <Chip
                  key={w.id}
                  label={`${w.entity_type}${w.action ? `/${w.action}` : ''}`}
                  active={entityType === w.entity_type && action === w.action}
                  onClick={() => pick(w)}
                />
              ))}
          </div>
      ...
      <Field
        label={tr('Type d’entité', 'Entity type')}
        value={entityType}
        onChange={setEntityType}
        placeholder="risk_acceptance"
      />
      <Field label="Action" value={action} onChange={setAction} placeholder="accept" />
      <Field
        label={tr('ID de l’entité (optionnel)', 'Entity ID (optional)')}
        value={entityId}
        onChange={setEntityId}
        placeholder="risk-uuid"
      />
      <Field
        label={tr('Titre', 'Title')}
        value={title}
        onChange={setTitle}
        placeholder={tr('Accepter le risque résiduel Log4Shell', 'Accept Log4Shell residual risk')}
      />
      <Field
...
function DelegationsView() {
  ...
  const revoke = async (d: Delegation) => {
    ...
    try {
      await revokeDelegation.mutateAsync(d.id);
      toast.success(tr('Révoquée', 'Revoked'));
    } catch {
      toast.error(tr('Échec', 'Failed'));
    }
  };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="text-[12.5px]" style={{ color: 'var(--fg-secondary)' }}>
          {tr(
            'Confiez temporairement vos droits à un collègue (absence, congés).',
            'Temporarily hand your rights to a colleague (leave, absence).',
          )}
        </div>
        <Btn
          label={tr('Nouvelle délégation', 'New delegation')}
          icon={UserPlus}
          primary
      ...
      {isLoading && delegations.length === 0 ? (
        ...
        <EmptyState
          ...
          description={tr(
            'Une délégation confie temporairement vos droits à un collègue — pendant un congé, par exemple. Elle expire d’elle-même à la date de fin.',
            'A delegation temporarily hands your rights to a colleague — during leave, for instance. It expires on its own end date.',
          )}
        />
      ) : (
              ...
              <div className="flex-1 min-w-[200px]">
                <div className="text-[13px]">
                  <span className="font-semibold">
                    {d.delegator_email || d.delegator_id.slice(0, 8)}
                  </span>
                  <ArrowRight
                    size={12}
                  ...
                  />
                  <span className="font-semibold">
                    {d.delegate_email || d.delegate_id.slice(0, 8)}
                  </span>
                </div>
                <div className="text-[12px] mt-0.5" style={{ color: 'var(--fg-secondary)' }}>
                  {d.permissions.join(', ')} · {fmt(d.starts_at)} → {fmt(d.ends_at)}
                  {d.reason ? ` · ${d.reason}` : ''}
                </div>
              </div>
              {d.status === 'active' && (
                <Btn label={tr('Révoquer', 'Revoke')} icon={Trash2} onClick={() => revoke(d)} />
              )}
            </div>
          ))}
        </Card>
...
function CreateDelegationModal({ onClose }: { onClose: () => void }) {
  ...
  const submit = async () => {
    const perms = permissions
      ...
      .map((p) => p.trim())
      .filter(Boolean);
    if (!delegateId || perms.length === 0 || !endsAt) {
      toast.error(
        tr(
          'Délégataire, permissions et date de fin requis',
          'Delegate, permissions and end date required',
        ),
      );
      return;
    }
  ...
  return (
    <ModalShell
      title={tr('Nouvelle délégation', 'New delegation')}
      onClose={onClose}
      onSubmit={submit}
      submitLabel={tr('Créer', 'Create')}
    >
      <Field
        label={tr('ID de l’utilisateur délégataire', 'Delegate user ID')}
        value={delegateId}
      ...
      <Field
        label={tr(
          'Permissions (séparées par des virgules, ou *)',
          'Permissions (comma-separated, or *)',
        )}
        value={permissions}
        onChange={setPermissions}
      />
      <Field
        label={tr('Raison', 'Reason')}
        value={reason}
        onChange={setReason}
        placeholder={tr('Congés annuels', 'Annual leave')}
      />
      <div>
        <label className="text-[12px] font-semibold">
          {tr('Fin de la délégation', 'Delegation ends')}
        </label>
        <input
          type="datetime-local"
          value={endsAt}
          onChange={(e) => setEndsAt(e.target.value)}
          className="w-full mt-1 h-9 px-2.5 rounded-[9px] bg-transparent text-[13px]"
          style={{ border: '1px solid var(--border-strong)' }}
        />
      </div>
    </ModalShell>
...
function WorkflowsView() {
  ...
  const remove = async (w: ApprovalWorkflow) => {
    ...
    try {
      await deleteWorkflow.mutateAsync(w.id);
      toast.success(tr('Supprimé', 'Deleted'));
    } catch {
      toast.error(tr('Échec', 'Failed'));
    }
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="text-[12.5px]" style={{ color: 'var(--fg-secondary)' }}>
          {tr(
            'Définissez des chaînes d’approbation (ex : accepter un risque = propriétaire + CISO).',
            'Define approval chains (e.g. accept a risk = owner + CISO).',
          )}
        </div>
        <Btn
          label={tr('Nouveau workflow', 'New workflow')}
          icon={Plus}
          primary
          onClick={() => setShowCreate(true)}
        />
      ...
      {isLoading && workflows.length === 0 ? (
        ...
        <EmptyState
          ...
          description={tr(
            'Un workflow impose une chaîne d’approbations sur une action sensible (accepter un risque, par exemple) : à chaque étape, un rôle habilité doit signer.',
            'A workflow enforces an approval chain over a sensitive action (accepting a risk, say): at each step an eligible role must sign off.',
          )}
        />
      ) : (
        <div className="space-y-3">
                ...
                <div className="flex-1 min-w-[240px]">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    ...
                    {w.expires_in_hours > 0 && (
                      ...
                      >
                        {tr(
                          `expire après ${w.expires_in_hours} h`,
                          `expires after ${w.expires_in_hours}h`,
                        )}
                      </span>
                    )}
                  </div>
...
function CreateWorkflowModal({ onClose }: { onClose: () => void }) {
  ...
  const [steps, setSteps] = useState<
    Array<{ name: string; approver_role: string; min_approvals: number; quorum_percent?: number }>
  >([
  ...
  const submit = async () => {
    ...
    if (!name || !requestType || clean.length === 0) {
      toast.error(
        tr(
          'Nom, type de demande et au moins une étape requis',
          'Name, request type and at least one step required',
        ),
      );
      return;
    }
  ...
  return (
    <ModalShell
      title={tr('Nouveau workflow', 'New workflow')}
      onClose={onClose}
      onSubmit={submit}
      submitLabel={tr('Créer', 'Create')}
    >
      <Field
        ...
        value={name}
        onChange={setName}
        placeholder={tr('Acceptation de risque', 'Risk acceptance')}
      />
      <div>
        ...
        <div className="flex-1">
          ...
          >
            <option value="sequential">
              {tr('Séquentiel — une étape après l’autre', 'Sequential — one step after another')}
            </option>
            <option value="parallel">
              {tr('Parallèle — toutes les étapes en même temps', 'Parallel — every step at once')}
            </option>
          </select>
        </div>
        <div className="w-[170px]">
          <label className="text-[12px] font-semibold">
            {tr('Expire après (h)', 'Expires after (h)')}
          </label>
          <input
            type="number"
            min={0}
            ...
            className="w-full mt-1 h-9 px-2.5 rounded-[9px] bg-transparent text-[13px] mono"
            style={{ border: '1px solid var(--border-strong)' }}
          />
        </div>
      </div>
      <p className="text-[11.5px] -mt-2" style={{ color: 'var(--fg-secondary)' }}>
        {tr(
          '0 = n’expire jamais. Sinon, une demande non décidée dans le délai se clôt comme « expirée » — ce qui n’est pas un refus.',
      ...
      <div>
        <div className="flex items-center justify-between">
          <label className="text-[12px] font-semibold">
            {tr('Étapes d’approbation', 'Approval steps')}
          </label>
          <button
            onClick={addStep}
          ...
          {steps.map((s, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <span
                ...
                style={{ color: 'var(--fg-secondary)', width: 18 }}
              >
                {i + 1}
              </span>
              <input
                ...
                placeholder={tr('Nom de l’étape', 'Step name')}
                className="flex-1 h-8 px-2 rounded-[8px] bg-transparent text-[12.5px]"
                style={{ border: '1px solid var(--border-strong)' }}
              />
              <input
                value={s.approver_role}
                onChange={(e) => setStep(i, { approver_role: e.target.value })}
                placeholder={tr('rôle', 'role')}
                className="w-24 h-8 px-2 rounded-[8px] bg-transparent text-[12.5px]"
                style={{ border: '1px solid var(--border-strong)' }}
              />
              <input
                type="number"
                ...
                onChange={(e) =>
                  setStep(i, { min_approvals: Math.max(1, Number(e.target.value) || 1) })
                }
    ...
    <div>
      ...
      {textarea ? (
        <textarea
          ...
          rows={3}
          className="w-full mt-1 px-2.5 py-2 rounded-[9px] bg-transparent text-[13px]"
          style={{ border: '1px solid var(--border-strong)' }}
        />
      ) : (
        <input
