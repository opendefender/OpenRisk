...
import {
  Workflow, Plus, Pencil, Trash2, Timer, AlertTriangle, Siren,
  Activity, ChevronRight, ArrowRight, Bell, MessageSquare, Mail, FlaskConical,
  Pause, PlayCircle, RotateCcw, Sparkles, Webhook, Smartphone, CheckCircle2,
  XCircle, Loader2, HelpCircle,
} from 'lucide-react';
...
import {
  ...
  useChannelConfig, useAutomationMutations, useAutomationState,
  useAutomationTemplates, useChannelCatalogue,
} from './useAutomation';
import type {
  AutomationRule, RuleHealth, NotifyChannel, ChannelTestResult,
    ...
    return (
      <>
        <EmptyState icon={Workflow}
          ...
          primaryAction={canWrite ? <Btn label={tr('Partir d’un modèle', 'Start from a template')} icon={Sparkles} primary onClick={() => setShowTemplates(true)} /> : undefined}
          secondaryAction={canWrite ? <Btn label={tr('Créer de zéro', 'Build from scratch')} icon={Plus} onClick={onNew} /> : undefined}
        />
  ...
  const toggle = async (r: AutomationRule) => {
    try {
      if (r.enabled) {
        ...
        if (!reason.trim()) { toast.error(tr('Une raison est requise', 'A reason is required')); return; }
          ...
          <Card key={r.id} className="or-fadeup" style={{ padding: '14px 16px', animationDelay: `${Math.min(idx * 40, 240)}ms` }}>
            <div className="flex items-start gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {canWrite && (
                  <>
                    <button title={r.enabled ? tr('Suspendre', 'Suspend') : tr('Activer', 'Enable')} onClick={() => toggle(r)}
                      className="w-8 h-8 rounded-[8px] flex items-center justify-center"
                      style={{ background: 'var(--bg-hover)', color: r.enabled ? 'var(--medium)' : 'var(--low)' }}>
...
function TemplateGallery({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,.35)' }}>
      <div className="w-full max-w-[720px] max-h-[85vh] flex flex-col rounded-[14px] or-scalein"
        ...
        <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg"><XCircle size={18} /></button>
        </div>
                  ...
                  {(template.requires_channels || template.requires_ticketing) && (
                    <p className="text-[11.5px] mt-1.5 inline-flex items-center gap-1.5" style={{ color: 'var(--medium)' }}>
                      ...
                      {[template.requires_channels && tr('un canal d’alerte', 'an alert channel'),
                        template.requires_ticketing && tr('une intégration ITSM', 'an ITSM integration')]
                        .filter(Boolean).join(' · ')}
                    </p>
                  )}
                </div>
                <Btn label={tr('Ajouter', 'Add')} icon={Plus} primary
                  onClick={() => adopt(template.key, lang === 'fr' ? template.name : template.name_en)} />
              </div>
            </Card>
          ))}
        </div>
...
function HistoryView() {
  ...
  const replay = async (id: string) => {
    ...
    } catch { toast.error(tr('Le rejeu a échoué', 'Replay failed')); }
  };
        ...
        return (
          <Card key={e.id} style={{ padding: '10px 14px' }}>
            <div className="flex items-center gap-2.5">
              <button onClick={() => setOpenId(open ? null : e.id)} className="flex-1 min-w-0 flex items-center gap-2.5 text-left">
            ...
            {open && (
              <div className="mt-3 pl-6 space-y-3">
                {e.error && (
                  <div className="text-[12px] rounded-[8px] px-2.5 py-1.5"
                ...
                <div className="space-y-1">
                  {(e.steps ?? []).map((st, i) => (
                    <div key={i} className="flex items-start gap-2 text-[12px]">
                      ...
                      <span className="font-semibold shrink-0" style={{ color: stepColor(st.status), minWidth: 96 }}>{st.action}</span>
                      <span className="text-ink-muted flex-1">{st.detail}</span>
                      <span className="mono text-[10.5px] text-ink-muted shrink-0">{st.duration_ms} ms</span>
                    </div>
                  ))}
...
// ---------------------------------------------------------------------------
const CHANNEL_ROWS: {
  key: NotifyChannel; icon: typeof Bell; title: string; hint: { fr: string; en: string };
}[] = [
  { key: 'in_app', icon: Bell, title: 'In-app', hint: { fr: 'Toujours disponible — la cloche de l’application.', en: 'Always available — the in-app bell.' } },
  { key: 'email', icon: Mail, title: 'Email', hint: { fr: 'Adresse de repli quand une alerte par rôle ne trouve personne.', en: 'Fallback address when a role-based alert resolves to nobody.' } },
  { key: 'slack', icon: MessageSquare, title: 'Slack', hint: { fr: 'Webhook entrant Slack.', en: 'Slack incoming webhook.' } },
  { key: 'teams', icon: MessageSquare, title: 'Microsoft Teams', hint: { fr: 'Webhook entrant Teams.', en: 'Teams incoming webhook.' } },
  { key: 'webhook', icon: Webhook, title: 'Webhook', hint: { fr: 'N’importe quel endpoint HTTPS. Signé (HMAC-SHA256) si un secret est fourni.', en: 'Any HTTPS endpoint. Signed (HMAC-SHA256) when a secret is set.' } },
  { key: 'sms', icon: Smartphone, title: 'SMS', hint: { fr: 'Passerelle HTTP générique — les noms de champs sont configurables, aucun opérateur n’est imposé.', en: 'Generic HTTP gateway — field names are configurable, no operator is assumed.' } },
];
...
function ChannelsView({ canWrite }: { canWrite: boolean }) {
  ...
  const on = (k: NotifyChannel): boolean => {
    ...
    switch (k) {
      case 'slack': return cfg?.slack_enabled ?? false;
      case 'teams': return cfg?.teams_enabled ?? false;
      case 'email': return cfg?.email_enabled ?? true;
      case 'webhook': return cfg?.webhook_enabled ?? false;
      case 'sms': return cfg?.sms_enabled ?? false;
      default: return true;
    }
  };
  ...
  const save = async () => {
    try {
      await saveChannels.mutateAsync({
        slack_enabled: on('slack'), slack_webhook_url: form.slack_url || undefined,
        teams_enabled: on('teams'), teams_webhook_url: form.teams_url || undefined,
        email_enabled: on('email'), default_email: val('default_email', cfg?.default_email ?? ''),
        webhook_enabled: on('webhook'), webhook_url: form.webhook_url || undefined,
        webhook_secret: form.webhook_secret || undefined,
        sms_enabled: on('sms'), sms_gateway_url: form.sms_gateway_url || undefined,
      ...
      setForm((f) => ({ ...f, slack_url: '', teams_url: '', webhook_url: '', webhook_secret: '', sms_gateway_url: '', sms_api_key: '' }));
    } catch { toast.error(tr('Échec', 'Failed')); }
  };
  ...
  const runTest = async (ch: NotifyChannel) => {
    ...
    } catch { toast.error(tr('Le test n’a pas pu s’exécuter', 'The test could not run')); }
    finally { setTestingCh(null); }
  };

  const configured = (ch: NotifyChannel) => catalogue.find((c) => c.channel === ch)?.configured ?? false;

  const fields = (ch: NotifyChannel) => {
    switch (ch) {
      case 'email':
        return <input className={inputCls} style={inputStyle} placeholder={tr('Email de repli (SOC)', 'Fallback email (SOC)')}
          value={val('default_email', cfg?.default_email ?? '')} onChange={(e) => set('default_email', e.target.value)} disabled={!canWrite} />;
      case 'slack':
        return <input className={inputCls} style={inputStyle} placeholder={cfg?.has_slack ? tr('Configuré — coller pour remplacer', 'Configured — paste to replace') : 'https://hooks.slack.com/services/…'}
          value={form.slack_url ?? ''} onChange={(e) => set('slack_url', e.target.value)} disabled={!canWrite} />;
      case 'teams':
        return <input className={inputCls} style={inputStyle} placeholder={cfg?.has_teams ? tr('Configuré — coller pour remplacer', 'Configured — paste to replace') : 'https://outlook.office.com/webhook/…'}
          value={form.teams_url ?? ''} onChange={(e) => set('teams_url', e.target.value)} disabled={!canWrite} />;
      case 'webhook':
        return (
          <div className="space-y-2">
            <input className={inputCls} style={inputStyle} placeholder={cfg?.has_webhook ? tr('Configuré — coller pour remplacer', 'Configured — paste to replace') : 'https://example.com/hooks/openrisk'}
              value={form.webhook_url ?? ''} onChange={(e) => set('webhook_url', e.target.value)} disabled={!canWrite} />
            <input className={inputCls} style={inputStyle} placeholder={tr('Secret de signature (optionnel)', 'Signing secret (optional)')}
              value={form.webhook_secret ?? ''} onChange={(e) => set('webhook_secret', e.target.value)} disabled={!canWrite} />
          </div>
        );
      case 'sms':
        return (
          <div className="space-y-2">
            <input className={inputCls} style={inputStyle} placeholder={cfg?.has_sms ? tr('Passerelle configurée — coller pour remplacer', 'Gateway configured — paste to replace') : 'https://api.operator.example/sms'}
              value={form.sms_gateway_url ?? ''} onChange={(e) => set('sms_gateway_url', e.target.value)} disabled={!canWrite} />
            <input className={inputCls} style={inputStyle} placeholder={tr('Clé API', 'API key')}
              value={form.sms_api_key ?? ''} onChange={(e) => set('sms_api_key', e.target.value)} disabled={!canWrite} />
            <input className={inputCls} style={inputStyle} placeholder={tr('Destinataires (+237…, séparés par des virgules)', 'Recipients (+237…, comma separated)')}
              value={val('sms_recipients', cfg?.sms_recipients ?? '')} onChange={(e) => set('sms_recipients', e.target.value)} disabled={!canWrite} />
            <div className="grid grid-cols-3 gap-2">
              <input className={inputCls} style={inputStyle} placeholder={tr('champ « to »', '"to" field')}
                value={val('sms_to_field', cfg?.sms_to_field ?? '')} onChange={(e) => set('sms_to_field', e.target.value)} disabled={!canWrite} />
              <input className={inputCls} style={inputStyle} placeholder={tr('champ « message »', '"message" field')}
                value={val('sms_text_field', cfg?.sms_text_field ?? '')} onChange={(e) => set('sms_text_field', e.target.value)} disabled={!canWrite} />
              <input className={inputCls} style={inputStyle} placeholder={tr('expéditeur', 'sender')}
                value={val('sms_sender', cfg?.sms_sender ?? '')} onChange={(e) => set('sms_sender', e.target.value)} disabled={!canWrite} />
            </div>
          </div>
        );
      default:
  ...
  return (
    <div className="max-w-[680px] space-y-3">
      <div className="text-[12.5px] text-ink-muted">
        {tr('Les secrets sont en écriture seule : ils ne sont jamais réaffichés. « Tester » envoie un vrai message et vous rend l’erreur exacte s’il n’arrive pas.',
      ...
      {CHANNEL_ROWS.map(({ key, icon: Icon, title, hint }) => {
        ...
        return (
          <div key={key} className="rounded-[12px] p-3.5" style={{ border: '1px solid var(--border-strong)', background: 'var(--bg-elevated)' }}>
            <div className="flex items-center gap-2.5 flex-wrap">
              {key !== 'in_app' ? (
                <input type="checkbox" checked={on(key)} disabled={!canWrite}
            ...
            {res && (
              <div className="mt-2.5 ml-[27px] text-[12px] flex items-start gap-1.5"
                ...
                {res.delivered ? <CheckCircle2 size={14} className="shrink-0 mt-0.5" /> : <XCircle size={14} className="shrink-0 mt-0.5" />}
                <span>
                  {res.detail}
