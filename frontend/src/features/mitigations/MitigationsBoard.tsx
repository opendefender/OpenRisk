...
//
// Mitigations Kanban (OpenRisk.dc.html §6.4): 4 columns (To do → In progress →
// In review → Done), each card carries a criticality dot + linked risk id, a
// progress bar and an assignee/deadline row (red + clock when overdue).

import { useState } from 'react';
import { Plus, Clock } from 'lucide-react';
import { PageFrame, PageHeader, Btn, Avatar } from '../../shared/ui';
import { critColor } from '../../shared/riskColors';
import { MITIGATIONS, type FxMiti } from '../../shared/fixtures';
import { useUIStrings } from '../../shared/uiStrings';

export function MitigationsBoard() {
  const L = useUIStrings();
  const cols: [keyof typeof MITIGATIONS, string, string][] = [
  ...
  return (
    <PageFrame wide>
      <PageHeader title={L.mitiTitle} actions={<Btn label={L.addPlan} icon={Plus} primary />} />
          ...
          <div key={key} className="rounded-[14px] p-3" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', minHeight: 200 }}>
            <div className="flex items-center justify-between mb-3 px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold text-ink-muted">{MITIGATIONS[key].length}</span>
              </div>
              <button className="w-[22px] h-[22px] rounded-md flex items-center justify-center text-ink-muted hover:bg-hover transition-colors"><Plus size={14} /></button>
            </div>
            {MITIGATIONS[key].map((c) => <KanbanCard key={c.id} c={c} />)}
          </div>
        ))}
      </div>
      </div>
...
function KanbanCard({ c }: { c: FxMiti }) {
