...
export function ScanPreviewPage() {
  ...
  const setCrit = (extId: string, c: AssetCriticality) =>
    setSelected((s) => ({ ...s, [extId]: c }));

  const doImport = () => {
    const selections: ImportSelection[] = Object.entries(selected).map(
      ([external_id, criticality]) => ({ external_id, criticality }),
    );
    if (!selections.length)
      return toast.error(tr('Sélectionnez au moins un actif', 'Select at least one asset'));
    importPreview.mutate(selections, {
      onSuccess: (r) => {
        toast.success(
          tr(`${r.assets_imported} actif(s) importé(s)`, `Imported ${r.assets_imported} asset(s)`),
        );
        setSelected({});
        navigate('/assets');
      },
      onError: (e) =>
        toast.error(
          (e as { response?: { data?: { error?: string } } })?.response?.data?.error ??
            tr('Échec de l’import', 'Import failed'),
        ),
    });
  };

  const doIgnore = () => {
    if (!confirm(tr('Ignorer et supprimer ce preview ?', 'Ignore and discard this preview?')))
      return;
    ignorePreview.mutate(undefined, {
      onSuccess: () => {
        toast.success(tr('Preview ignoré', 'Preview discarded'));
    ...
    return (
      <PageFrame>
        <ErrorState
          ...
          description={tr(
            'Il a peut-être expiré (48h) ou a déjà été importé.',
            'It may have expired (48h) or already been imported.',
          )}
  ...
  const sourceLabel = preview.agent_name
    ? tr(
        `Scan exécuté par l'Agent ${preview.agent_name} (sur site)`,
        `Scan run by Agent ${preview.agent_name} (on-prem)`,
      )
  ...
  const tabs: { key: Tab; label: string; icon: typeof Boxes; n: number }[] = [
    ...
    {
      key: 'mitigations',
      label: tr('Mitigations auto', 'Auto-mitigations'),
      icon: Wrench,
      n: mitigations.length,
    },
  ];
  ...
  return (
    <PageFrame wide>
      <button
        ...
        className="inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-ink-soft hover:text-ink mb-3"
      >
              ...
              <Btn
                label={
                  importPreview.isPending
                    ? tr('Import…', 'Importing…')
                    : tr(`Importer (${selCount})`, `Import selection (${selCount})`)
                }
                icon={Download}
                primary
                onClick={doImport}
      ...
      >
        {m.cloud ? (
          <Cloud size={16} style={{ color: m.color }} />
        ) : (
          <Server size={16} style={{ color: m.color }} />
        )}
        <span className="text-[13px] font-medium text-ink">{sourceLabel}</span>
        <span className="text-[12px] text-ink-soft ml-auto inline-flex items-center gap-1.5">
          <ShieldCheck size={14} style={{ color: 'var(--low)' }} />{' '}
          {tr('Rien n’est écrit tant que vous n’importez pas', 'Nothing is saved until you import')}
        </span>
      </div>
