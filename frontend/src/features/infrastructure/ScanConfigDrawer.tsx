...
export function ScanConfigDrawer({
  ...
  const submit = async () => {
    ...
    const input: CreateScanConfigInput = {
      name: name.trim(),
      provider,
    ...
    if (isCloud) {
      ...
      const tg = targets
        ...
        .filter(Boolean);
      if (!tg.length)
        return toast.error(tr('Au moins une cible est requise', 'At least one target is required'));
      input.targets = tg;
    }
    try {
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-4">
          <Field label={tr('Nom', 'Name')}>
            <input
              ...
              placeholder={
                isCloud
                  ? tr('Production AWS', 'Production AWS')
                  : tr('Sweep réseau siège', 'HQ network sweep')
              }
            />
          </Field>
          ...
          {isCloud ? (
              ...
              {SCOPE_HINTS[provider] && (
                <Field label={tr(SCOPE_HINTS[provider]!.fr, SCOPE_HINTS[provider]!.en)}>
                  <input
                    ...
                    placeholder={SCOPE_HINTS[provider]!.placeholder}
                  />
                </Field>
              )}
              <div
                className="flex items-start gap-2 text-[12px] text-ink-soft rounded-lg px-3 py-2.5"
                style={{ background: 'var(--bg-hover)' }}
              >
                <ShieldCheck size={15} className="shrink-0 mt-px" style={{ color: 'var(--low)' }} />
                <span>
                  {tr(
                    'Chiffré en AES-256-GCM, déchiffré uniquement au moment du scan.',
                    'Encrypted with AES-256-GCM, decrypted only at scan time.',
                  )}
                </span>
              </div>
            </>
            ...
            <>
              <Field
                label={tr(
                  'Cibles (une par ligne — CIDR ≤ /24 ou hôte)',
                  'Targets (one per line — CIDR ≤ /24 or host)',
                )}
              >
                <textarea
                  style={monoStyle}
              ...
              >
                ...
                <span>
                  {tr(
                    'Le scan est exécuté par un Agent sur site (nmap/osquery), jamais par le SaaS. Déployez un agent après la création.',
                    'Scans run on an on-prem Agent (nmap/osquery), never on the SaaS. Deploy an agent after creating.',
                  )}
                </span>
              </div>
            </>
        ...
        >
          ...
          <Btn
            label={submitting ? tr('Création…', 'Creating…') : tr('Créer', 'Create')}
            primary
            onClick={submit}
          />
        </div>
