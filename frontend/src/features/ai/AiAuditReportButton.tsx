      ...
      {open && (
        ...
        >
          ...
          >
            ...
            >
              <div
                className="w-8 h-8 rounded-[9px] flex items-center justify-center text-accent-strong shrink-0"
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[14px] font-semibold text-ink truncate">
                  {tr('Rapport d’audit — IA', 'Audit report — AI')}
                </div>
              ...
              {r && (
                <button
                  onClick={copyText}
                  className="w-8 h-8 rounded-lg flex items-center justify-center text-ink-soft hover:text-ink"
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              ...
              {report.isError && (
                <div className="py-10 text-center text-[13px]" style={{ color: 'var(--critical)' }}>
                  {tr('La génération a échoué.', 'Generation failed.')}
                </div>
              )}
              {r && (
                <div className="space-y-4">
                  <Section
                    title={tr('Synthèse exécutive', 'Executive summary')}
                  ...
                  >
                    ...
                    <span className="font-semibold">{report.data?.generated_by}</span>
                  </div>
                </div>
              )}
            </div>
...
function Section({ title, body }: { title: string; body: string }) {
  ...
  return (
    <div>
      ...
      <div className="text-[13px] text-ink leading-relaxed" style={{ whiteSpace: 'pre-line' }}>
        {body}
      </div>
    </div>
  );
