  ...
  return (
    <div className="max-w-[900px] mx-auto px-6 py-7">
      <div className="flex items-center gap-3 mb-1.5">
        <div
          className="w-9 h-9 rounded-[11px] flex items-center justify-center text-accent-strong"
          style={{ background: 'var(--accent-soft)' }}
        >
          <Sparkles size={19} />
        </div>
        <h1 className="disp text-[22px] font-bold text-ink">
          {tr('Détection de risques émergents', 'Emerging risk detection')}
        </h1>
      </div>
      ...
      >
        ...
        <div className="flex items-center justify-between mt-3">
          <span className="text-[11.5px] text-ink-muted">
            {text.length} {tr('caractères', 'characters')}
          </span>
          <button
            onClick={analyze}
          ...
          >
            ...
            {detect.isPending
              ...
              : tr('Analyser avec l’IA', 'Analyse with AI')}
          </button>
        </div>
      </div>

      {detect.isError && (
        <div className="text-[13px]" style={{ color: 'var(--critical)' }}>
          {tr('L’analyse a échoué. Réessayez.', 'Analysis failed. Please try again.')}
        </div>
      )}
        ...
        <div style={{ animation: 'or-fadeup .25s ease' }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] text-ink-muted">
              {tr('Généré par', 'Generated by')} {detect.data?.generated_by}
            </span>
          </div>
          {res.risks.length === 0 ? (
            <div className="text-center py-10 text-[13px] text-ink-soft">
              {tr('Aucun risque émergent détecté.', 'No emerging risk detected.')}
            </div>
          ) : (
            <div
            ...
            >
              {res.risks.map((rk: EmergingRisk, i: number) => (
                <div
                  key={i}
                  className="p-4 rounded-[13px]"
                  style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }}
                >
                  <div className="flex items-start gap-2 mb-2">
                    <AlertTriangle
                      size={16}
                      style={{ color: SEV_COLOR[rk.severity] ?? 'var(--accent)' }}
                      className="shrink-0 mt-0.5"
                    />
                    <div className="text-[14px] font-semibold text-ink leading-snug flex-1">
                      {rk.title}
                    </div>
                  </div>
                  <div className="text-[12.5px] text-ink-soft leading-relaxed mb-3">
                    {rk.description}
                  </div>
                  <div className="flex items-center gap-2 flex-wrap text-[11px]">
                    <span
                      ...
                      style={{
                        ...
                        color: SEV_COLOR[rk.severity] ?? 'var(--accent)',
                    ...
                    <span
                      className="px-2 py-0.5 rounded-full text-ink-soft"
                      style={{
                        background: 'var(--bg-elevated)',
                        border: '1px solid var(--border)',
                      }}
                    ...
                    <span className="text-ink-muted ml-auto">
                      P {(rk.suggested_probability * 100).toFixed(0)}% · I{' '}
                      {rk.suggested_impact.toFixed(1)}
                    </span>
                  </div>
                  {rk.rationale && (
                    <div className="text-[11.5px] text-ink-muted italic mt-2 leading-relaxed">
                      {rk.rationale}
                    </div>
                  )}
                </div>
