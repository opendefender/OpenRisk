...
export function WarRoom() {
  ...
  const closeIncident = () => {
    ...
    updateIncident.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(tr('Incident clos', 'Incident closed'));
          setConfirmClose(false);
          navigate('/incidents');
        },
        onError: () => {
          toast.error(tr('Clôture échouée', 'Close failed'));
          setConfirmClose(false);
        },
      },
    );
    ...
    return (
      <div className="p-6">
        <button
          ...
          className="inline-flex items-center gap-1.5 text-[13px] font-medium text-ink-soft hover:text-ink transition-colors mb-4"
        >
          <ArrowLeft size={15} /> {tr('Incidents', 'Incidents')}
        </button>
        <EmptyState
          variant="no-results"
          title={tr('Incident introuvable', 'Incident not found')}
          description={tr(
            'Cet incident n’existe plus ou ne vous est pas accessible.',
            'This incident no longer exists or is not accessible to you.',
          )}
        />
      </div>
    );
  ...
  const parts = [
    incident.assigned_to
      ? {
          ...
          role: tr('Assigné', 'Assignee'),
          init: initialsOf(incident.assigned_to),
        }
      : null,
    incident.reported_by
      ? {
          name: incident.reported_by,
          role: tr('Déclarant', 'Reporter'),
          init: initialsOf(incident.reported_by),
        }
      : null,
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 58px)' }}>
      {/* top bar */}
      <div
        className="shrink-0 flex items-center gap-[18px] px-6 py-4 flex-wrap"
        style={{
          borderBottom: '1px solid var(--border)',
          background: `linear-gradient(90deg,color-mix(in srgb,${sev.color} 7%,transparent),transparent 60%)`,
        }}
      >
        <button
          onClick={() => navigate('/incidents')}
          className="w-9 h-9 rounded-[9px] flex items-center justify-center text-ink-soft hover:bg-hover hover:text-ink transition-colors shrink-0"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2.5 mb-1 flex-wrap">
            <span
              className="inline-flex items-center gap-1.5 text-[10.5px] font-bold tracking-[.06em] uppercase px-[9px] py-[3px] rounded-full"
              style={{
                color: st.color,
            ...
            >
              {!closed && (
                <span
                  ...
                  style={{ background: st.color, animation: 'or-pulsedot 1.4s infinite' }}
                />
              )}
              {tr(st.fr, st.en)}
            </span>
        ...
        <div className="text-center">
          <div className="disp mono text-[22px] font-bold text-ink leading-none">
            {elapsedParts(incident.created_at, frozenMs)}
          </div>
          <div className="text-[10.5px] text-ink-muted mt-[3px]">{tr('Durée', 'Elapsed')}</div>
        </div>
        <div className="text-center">
          <div className="text-[13px] font-bold" style={{ color: sev.color }}>
            {tr(sev.fr, sev.en)}
          </div>
        ...
        <div className="flex gap-2.5">
          {!closed && (
            <button
              ...
              className="h-9 px-[15px] rounded-[10px] text-[13px] font-semibold text-fg-on-solid inline-flex items-center gap-1.5 transition-all hover:brightness-110"
              style={{ background: 'var(--danger-solid)' }}
            >
              <Check size={16} /> {tr('Clore l’incident', 'Close incident')}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex min-h-0">
        ...
        >
          <div className="text-[11px] font-semibold uppercase tracking-[.05em] text-ink-muted mb-3.5">
            {tr('Participants', 'Responders')} · {parts.length}
          </div>
          {parts.length === 0 ? (
            <div className="text-[12px] text-ink-muted px-2 py-3 leading-relaxed">
              {tr(
                'Personne n’est encore assigné à cet incident.',
                'Nobody is assigned to this incident yet.',
              )}
            </div>
          ) : (
            parts.map((pp) => (
              <div
                key={pp.role}
                className="flex items-center gap-2.5 px-2 py-2 rounded-[10px] mb-0.5"
              >
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <div ref={chatRef} className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-3.5">
            <div
              className="self-center inline-flex items-center gap-2 text-[11px] text-ink-muted px-3 py-1.5 rounded-full"
            ...
            {timeline.map((e) => (
              <div
                key={e.id}
                className="self-center inline-flex items-center gap-2 text-[11.5px] text-ink-muted px-3 py-1.5 rounded-full"
                style={{ background: 'var(--bg-hover)' }}
              >
                <AlertTriangle size={13} /> {e.message}{' '}
                <span className="opacity-70">· {relTime(e.created_at, lang)}</span>
              </div>
            ))}
          </div>
          <div
        ...
        {/* Response task board — real, persisted, tenant-scoped
            (GET/POST/PUT /incidents/:id/actions). */}
        <div
          className="w-[290px] shrink-0 overflow-y-auto p-4 hidden lg:block"
        ...
        >
          ...
          {!closed && (
            <div className="mb-4 flex flex-col gap-2">
              ...
              >
                <Send size={14} />{' '}
                {create.isPending ? tr('Ajout…', 'Adding…') : tr('Ajouter', 'Add')}
              </button>
              {actionError && (
                <div role="alert" className="text-[11.5px]" style={{ color: 'var(--critical)' }}>
                  {actionError}
                </div>
              )}
            </div>
          ...
          {actionsLoading ? (
            ...
            <div className="text-[12px] leading-relaxed" style={{ color: 'var(--critical)' }}>
              ...
              <button onClick={() => void refetchActions()} className="ml-1 underline">
                {tr('Réessayer', 'Retry')}
              </button>
            </div>
          ) : actions.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {actions.map((a) => {
                ...
                const next: typeof a.status = done
                  ? 'pending'
                  : a.status === 'pending'
                    ? 'in_progress'
                    : 'completed';
        ...
        >
          ...
          >
            ...
            <div className="flex gap-2.5">
              <button
                onClick={() => setConfirmClose(false)}
                className="flex-1 h-[42px] rounded-[11px] text-[13.5px] font-semibold text-ink"
              ...
              <button
                ...
                className="flex-1 h-[42px] rounded-[11px] text-[13.5px] font-semibold text-fg-on-solid disabled:opacity-60"
                style={{ background: 'var(--danger-solid)' }}
              >
                {tr('Clore', 'Close')}
              </button>
