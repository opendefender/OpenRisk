...
export function IncidentDrawer({
  ...
  const remove = () => {
    if (
      !window.confirm(
        tr(`Supprimer l’incident « ${incident.title} » ?`, `Delete incident "${incident.title}"?`),
      )
    )
      return;
    deleteIncident.mutate(incident.id, {
      onSuccess: () => {
        toast.success(tr('Incident supprimé', 'Incident deleted'));
        onClose();
      },
  ...
  const inputCls =
    'w-full h-10 px-3.5 rounded-[10px] text-[13px] text-ink outline-none focus:border-accent transition-colors disabled:opacity-70';
    ...
    <>
      ...
      >
        ...
        >
            ...
            <div className="flex items-start gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span
                    className="inline-flex items-center gap-1.5 text-[11px] font-semibold px-[9px] py-[3px] rounded-full"
                    style={{
                      color: sevMeta(severity).color,
                      background: `color-mix(in srgb,${sevMeta(severity).color} 15%,transparent)`,
                    }}
                  >
                    <span
                      className="w-1.5 h-1.5 rounded-full"
                      style={{ background: sevMeta(severity).color }}
                    />
                  ...
                  <span
                    ...
                    style={{
                      ...
                      background: `color-mix(in srgb,${statusMeta(status).color} 12%,transparent)`,
                    }}
                  >
          ...
          <div className="flex-1 overflow-y-auto p-[22px] flex flex-col gap-4">
            ...
            {pmView?.required && (
              ...
              >
                <ClipboardList
                  ...
                  style={{
                    color:
                      pmView.post_mortem.status === 'published' ? 'var(--low)' : 'var(--critical)',
                  }}
                />
                <span className="text-[12.5px]" style={{ color: 'var(--fg-primary)' }}>
                  {pmView.post_mortem.status === 'published'
                    ? tr(
                        'Post-mortem publié — ouvrir le compte rendu.',
                        'Post-mortem published — open the record.',
                      )
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3.5 py-2.5 rounded-[10px] text-[13px] text-ink outline-none focus:border-accent transition-colors resize-none disabled:opacity-70"
                style={fieldStyle}
              />
            </label>
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder={tr('Responsable de la réponse', 'Response owner')}
                className={inputCls}
                style={fieldStyle}
              />
            </label>
            ...
            <div className="mt-1">
              <div className="flex items-center gap-1.5 mb-2.5">
                ...
                <span className={labelCls}>{tr('Chronologie', 'Timeline')}</span>
              </div>
              {tlLoading ? (
                <SkeletonRows rows={2} height={40} />
              ) : timeline.length === 0 ? (
                <div className="text-[12.5px] text-ink-muted">
                  {tr('Aucun événement.', 'No events yet.')}
                </div>
              ) : (
                    ...
                    <div key={e.id} className="flex gap-3">
                      ...
                      <div className="pb-3.5 flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-ink-muted mt-0.5">
                          {e.created_by || 'system'} · {relTime(e.created_at, lang)}
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {/* footer */}
          <div
            className="px-[22px] py-4 flex items-center gap-2.5"
          ...
          >
            ...
            {canWrite && (
              <button
                onClick={remove}
                className="w-9 h-9 rounded-[10px] inline-flex items-center justify-center transition-all hover:brightness-110"
                style={{
