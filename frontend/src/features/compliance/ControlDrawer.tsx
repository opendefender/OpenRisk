      ...
      {activeDrawerTab === 'details' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-fg-secondary">{control.description || '—'}</p>
          </div>
          {control.source_reference && (
            <div>
              ...
              <p className="mt-1 text-sm italic text-fg-secondary">{control.source_reference}</p>
            </div>
          )}
          <div>
            <label className="text-xs font-semibold uppercase tracking-[0.18em] text-fg-muted">
                ...
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-all disabled:opacity-50 ${
                    control.status === status
                      ? 'border-primary bg-primary/20 text-fg-primary'
                      : 'border-border-subtle bg-surface-1 text-fg-secondary hover:border-border-default'
                  }`}
                >
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          <div className="rounded-2xl border border-dashed border-border-default p-5">
            ...
            <textarea
              ...
              rows={2}
              className="mb-3 w-full rounded-xl border border-border-subtle bg-surface-0 px-3 py-2 text-sm text-fg-primary outline-none focus:ring-2 focus:ring-primary/40"
            />
            <Button
              type="button"
              variant="secondary"
                ...
                {evidences.map((evidence) => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="truncate text-sm text-fg-primary">{evidence.filename}</p>
                      {evidence.description && (
                        <p className="truncate text-xs text-fg-muted">{evidence.description}</p>
                      )}
                    </div>
                    <div className="flex shrink-0 items-center gap-1">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="rounded-lg p-2 text-fg-secondary hover:bg-danger/10 hover:text-danger-text transition-colors"
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
