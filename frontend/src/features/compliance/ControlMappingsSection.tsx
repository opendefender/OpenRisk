...
export function ControlMappingsSection({ control }: { control: ComplianceControl }) {
  ...
  const submit = () => {
    ...
    create.mutate(
      { source_control_id: control.id, target_control_id: targetControl, relation },
      {
        ...
        onError: () =>
          toast.error(tr('Ajout impossible (doublon ?)', 'Could not add (duplicate?)')),
      },
    );
  };
  ...
  return (
    <div>
      ...
      {canEdit && adding && (
        <div
          className="rounded-[11px] p-3 mb-3 flex flex-col gap-2"
          style={{ border: '1px solid var(--border-strong)', background: 'var(--bg-elevated)' }}
        >
          <div className="flex items-center justify-between">
            <div className="text-[12px] font-semibold text-ink">
              {tr('Lier à un autre référentiel', 'Link to another framework')}
            </div>
            <button onClick={() => setAdding(false)} className="text-ink-muted hover:text-ink">
              <X size={15} />
            </button>
          </div>
          <select
      ...
      {isLoading ? (
        ...
        <div className="text-center py-6 text-[12.5px] text-ink-muted">
          {tr(
            'Aucune correspondance. Reliez ce contrôle à un référentiel équivalent.',
            'No cross-mappings yet. Link this control to an equivalent framework.',
          )}
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {visibleMappings.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="flex items-center gap-3 px-3 py-2.5 rounded-[11px]"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11.5px] text-ink-muted truncate">
                    {fwName} · {relationLabel(m.relation, fr)}
                  </div>
                </div>
                {canEdit && (
                  <button
                    onClick={() =>
                      undoRemove(m.id, {
                        ...
                        onError: () =>
                          toast.error(tr('Suppression impossible', 'Could not remove')),
                      })
                    }
