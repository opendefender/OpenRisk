...
export function GapAnalysisPage() {
  ...
  const coverage =
    totalControls > 0 ? Math.round(((totalControls - totalGaps) / totalControls) * 100) : 100;
  ...
  const gaugeColor =
    coverage >= 70 ? 'var(--low)' : coverage >= 40 ? 'var(--high)' : 'var(--critical)';
  ...
  return (
    <PageFrame>
      ...
      <PageHeader
        ...
        actions={
          <Btn
            label={tr('Voir la conformité', 'View compliance')}
            icon={FileText}
            onClick={() => navigate('/compliance')}
          />
        }
      ...
      {isLoading ? (
        ...
        <Card>
          <EmptyState
            ...
            description={tr(
              'Importez un référentiel pour lancer une analyse d’écarts.',
              'Import a framework to run a gap analysis.',
            )}
            primaryAction={
              <Btn
                label={tr('Aller à la conformité', 'Go to compliance')}
                primary
                onClick={() => navigate('/compliance')}
              />
            }
        ...
        <Card>
          <EmptyState
            ...
            description={tr(
              'Tous les contrôles applicables sont implémentés sur vos référentiels.',
              'Every applicable control is implemented across your frameworks.',
            )}
          />
        </Card>
      ) : (
            ...
            {(data?.frameworks ?? [])
              ...
              .map((f) => (
                <FilterChip
                  ...
                  onClick={() => setFwFilter(f.framework_id)}
                  count={f.gaps}
                />
              ))}
          </div>
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span className="text-[14px] font-semibold text-ink group-hover:text-accent-strong-strong transition-colors">
                      {group.name}
                    </span>
                    <span
                    ...
                    >
                      {group.items.length} {tr('écart', 'gap')}
                      {group.items.length > 1 ? 's' : ''}
                    </span>
                  </div>
                  <ChevronRight
                    size={16}
                    className="text-ink-muted group-hover:text-accent-strong-strong transition-colors"
                  />
                </button>
                <div>
                  {group.items.map((g) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className="mono text-[12px] font-semibold text-ink-soft shrink-0 mt-0.5 w-[92px] truncate"
                          title={g.reference_code}
                        >
                          {g.reference_code}
                        </span>
                        <div className="flex-1 min-w-0">
                          ...
                          {g.source_reference && (
                            <div
                              className="text-[11.5px] text-ink-muted mt-0.5 truncate"
                              title={g.source_reference}
                            >
                              {g.source_reference}
                            </div>
                          )}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {g.evidence_count > 0 && (
                            <span className="text-[11px] text-ink-muted">
                              {g.evidence_count} {tr('preuve', 'evidence')}
                              {g.evidence_count > 1 ? 's' : ''}
                            </span>
                          )}
                          <span
