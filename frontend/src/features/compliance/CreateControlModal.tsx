...
export const CreateControlModal = ({ isOpen, onClose, frameworkId }: CreateControlModalProps) => {
  ...
  const {
    ...
    defaultValues: { reference_code: '', name: '', description: '' },
  });
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      toast.success(t('compliance.addControl'));
      handleClose();
    } catch {
      toast.error(t('errors.failedToCreateControl'));
    }
  };
            ...
            <div className="rounded-3xl border border-zinc-800 bg-zinc-950/95 p-6 shadow-2xl shadow-black/40">
              <div className="flex items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-semibold">{t('compliance.addControl')}</h2>
                </div>
                <button
                  type="button"
                  onClick={handleClose}
