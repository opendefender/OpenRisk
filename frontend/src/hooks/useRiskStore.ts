...
export interface Risk {
  ...
  source: string; // Important pour l'Ãtape d'intÃgration (THEHIVE, etc.)
  mitigations?: Mitigation[]; // Important pour le drawer de dÃtails
  created_at?: string;
...
export const useRiskStore = create<RiskStore>((set, get) => ({
  ...
  createRisk: async (payload) => {
    ...
    const optimistic: Risk = {
      id: tempId,
      title: payload.title || 'Nouvel ÃlÃment',
