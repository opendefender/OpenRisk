...
describe('Command', () => {
  ...
  it('finds an accented label from an unaccented query', async () => {
    ...
    expect(screen.getByRole('option', { name: /Règlement intérieur/ })).toBeInTheDocument();
  ...
  it('matches keywords as well as labels', async () => {
    ...
    expect(screen.getByRole('option', { name: /Export register/ })).toBeInTheDocument();
  });
  ...
  it('shows the empty message rather than an empty box', async () => {
    const user = userEvent.setup();
    render(<Command items={makeItems()} label="Commands" emptyMessage="Nothing found" />);
...
describe('OtpField', () => {
  ...
  it('normalises a dirty value it is GIVEN, not only one that is typed', () => {
    ...
    render(<OtpField label="Code" value="123 456" onValueChange={() => {}} />);

    const segments = screen.getAllByTestId('otp-segment');
    expect(segments.map((s) => s.textContent)).toEqual(['1', '2', '3', '4', '5', '6']);
