...
export const userFriendlyErrors = {
  ...
  LOADING: {
    ...
    THREATS:
      "Can't retrieve threat information right now. Please check your connection and try again.",
  ...
  PERMISSION: {
    AUDIT_LOGS:
      "You don't have permission to view audit logs. Contact an administrator for access.",
  ...
  VALIDATION: {
    ...
    INVALID_FORMAT: (fieldName: string) =>
      `${fieldName} has an invalid format. Please check and try again.`,
...
export const getUserFriendlyErrorMessage = (
  error: any,
  defaultMessage: string = userFriendlyErrors.GENERIC,
): string => {
  // If it's already a user-friendly message, return it
  if (typeof error === 'string') {
    // Check if it's already in our error messages
    if (
      Object.values(userFriendlyErrors).some(
        (category) => typeof category === 'object' && Object.values(category).includes(error),
      )
    ) {
      return error;
    }
