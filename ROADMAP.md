...
### M2 — Contenu réglementaire africain (le moat) 🟡 ISO 27001 fait le 08/07/2026 (branche `feat/m1-compliance-engine`), COBAC/BCEAO/ANSSI-CM non commencés
Dépend de : M1.
...
- [ ] **COBAC, BCEAO, ANSSI-CM, Loi camerounaise 2024/017** : **décision délibérée de ne pas les modéliser maintenant.** J'ai une confiance raisonnable dans la structure publique d'ISO 27001:2022 (largement documentée), mais nettement moins dans les numéros d'article/circulaire exacts de ces textes réglementaires régionaux — les inventer serait pire que ne pas avoir le référentiel du tout, dans un produit que de vrais responsables conformité vont utiliser. Catalogues déjà enregistrés dans `pkg/compliance/catalog_placeholders.go` avec `Available: false` et zéro contrôle, pour que l'UI les affiche en "Bientôt disponible" plutôt que de les cacher. **Prochaine étape : obtenir les textes source réels** (PDF/liens/texte) avant de modéliser un seul contrôle.
...
- **Acceptation partielle : ISO 27001 atteinte** (chargé, cité, testé, vérifié en direct : import réel de 93 contrôles via l'UI, voir preuve ci-dessous) ; **COBAC non atteint**, reste à faire dès que les textes source sont disponibles.
...
| **Wave 1** | **M1 Compliance engine ✅ fait + vérifié live 08/07/2026** (voir §3) + **M2 ISO 27001 ✅ fait + vérifié live 08/07/2026** (93 contrôles importables en un clic, voir §3) + COBAC/BCEAO/ANSSI-CM ❌ (délibérément reporté, textes source manquants) + Assets (🟡, plus avancé que prévu) + Reporting/Board (❌) + Incident (🟡, table manquante) + Offline (❌) + 3 écrans parfaits (❌, + nav coherence ajoutée) | 🟡 M1+M2(ISO) faits, priorité = COBAC (textes source) ou M3/M4 |
...
**La phrase à garder :** OpenRisk a maintenant un vrai *Compliance engine* bout-en-bout (M1) **et** son premier contenu réglementaire chargé (M2 : ISO 27001:2022, 93 contrôles, vérifié live) — pas seulement un excellent *Risk Register*. La suite de M2 (COBAC/BCEAO/ANSSI-CM) est délibérément en attente de vrais textes source plutôt que de contenu inventé. Un bug fondamental de multi-tenancy (`middleware.SetContext` jamais appelé en production, tenant_id silencieusement `Nil` sur 8 modules) a été découvert et corrigé le 08/07/2026 pendant cette vérification — voir §3 M2 pour le détail ; toute affirmation de statut antérieure à cette date sur Risk/Asset/Mitigation/Dashboard doit être considérée comme non fiable sur l'isolation multi-tenant tant qu'elle n'a pas été revérifiée.
