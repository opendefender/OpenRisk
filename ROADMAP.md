...
| **14.2 Vendor Risk Management** | ✅ (TPRM v1) | Ligne obsolète corrigée le 23/09/2026 (#483) : le package existe. `internal/application/tprm` (registre fournisseurs, chaîne fournisseur→actif→risque, modèles de questionnaire, envoi/relance/révocation d'évaluation, questionnaire public sans compte, scoring) + `pkg/vendorscore` + `features/tprm` (`VendorsPage`, `VendorDetailPage`, `VendorAssessmentPage`, `QuestionnaireTemplatesPage`/`Editor`, `VendorQuestionnairePage`) routés dans `App.tsx` (`/vendors`, `/vendors/questionnaires`) et `cmd/server/main.go` (`/vendors`, `/vendor-questionnaire-templates`). Relances J-7/J-3/J-1 balayées par un worker (`send_vendor_assessment_reminders.go`). Livré par #669, #673, #674, #681. | Gated `ent.FeatVendorRisk` (Business+). Pas de table `vendors` : un fournisseur est un actif. Auto-scoring continu et notation externe non couverts. |
...
| **17.6 Onboarding — tunnel guidé + Révélation de posture** | 🟡 | **Tunnel bloquant re-séquencé** (#438) : organisation → objectif → référentiel → **évaluation** → **couverture**. Les trois premières demandent, les deux dernières **rendent quelque chose de calculé** (matrice vivante, puis résiduel qui baisse à l'écran). `profile` est fusionné dans l'étape organisation ; `team` part sur la Révélation de posture. Aucun contrôle de fermeture, d'abandon ni d'échappement ; `OnboardingGuard` interdit `/app` jusqu'à `POST /onboarding/complete`. Un seul `GET /onboarding/state` résout tout le tunnel, décisions d'auto-skip comprises — une étape dont la donnée existe déjà n'est jamais rendue et disparaît du compteur (`internal/application/activation/onboarding.go`, `internal/domain/posture.go`). **Écran terminal `/posture`** : compté à partir des seules lignes du tenant, enregistre `posture.revealed` une fois (`internal/application/activation/posture.go`, `internal/infrastructure/repository/gorm_posture_repository.go`). **Écran `/onboarding/recognition`** pour les tenants déjà configurés. Score résiduel additif depuis `risk_control_mappings` (`pkg/scoring/residual.go`, ADR 0003). Catalogue de risques de démarrage FR+EN (`pkg/onboarding/starter.go`) : l'étape 2 en propose huit, l'utilisateur en choisit trois, et `POST /onboarding/starter-risks` les écrit comme de vraies lignes avec `source = "starter"` — le serveur relit l'énoncé dans son propre catalogue, le client n'envoie que des clés. Checklist d'activation à 7 étapes inchangée, désormais **masquable** par l'utilisateur. | Promesse d'activation : **Aha en < 8 min**, mesuré désormais sur `posture.revealed` et non plus sur le score du tableau de bord exécutif (D-010 ; série `openrisk_time_to_aha_seconds{aha_definition="v2"}`, la v1 est gelée). 🟡 et non ✅ : la suite E2E **a été exécutée** et les neuf tests d'activation passent, de même que l'a11y sur les sept routes du lot (zéro violation serious/critical) — mais **par lots**, la limite d'authentification (15 req/5 min/IP) épuisant son budget au septième test, et **le chemin d'écriture du résiduel reste à construire** : ADR 0003 est passé en `accepted` (D-042, 2026-09-10), donc `Risk.ResidualRisk` peut désormais être écrit, mais rien ne l'écrit encore. Passage à ✅ après reproduction sur un runner CI par `product-verifier`. Étapes « intégrations » et « première évaluation » : toujours hors périmètre (#234, #438). |
...
- [x] **UX-2 — Dashboards intelligents par rôle** ✅ 2026-07-23 (`feature/ux-role-dashboards`) — le dashboard
  ...
  **posture** (RSSI/risk/admin, layout existant) · **analyst** (vulns : KPI P1/KEV + file de priorité
  deep-linkée) · **audit** (couverture par référentiel + écarts + audits) · **exec** (score canonique — ex-cyber score A–F, retiré par #287 — + ALE
