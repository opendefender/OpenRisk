...
### M4 — Reporting officiel + Board Report ✅ (rapport de conformité 1 clic ✅ 09/07/2026 + Board Report IA/FCFA ✅ 10/07/2026, branche `feat/m4-compliance-report-pdf`)
- [x] **Rapport de conformité officiel (PDF, 1 clic)** — vérifié live le 09/07/2026 (backend réel, DB réelle, PDF rendu en images et inspecté). `GET /compliance/frameworks/{id}/report?locale=fr|en` renvoie un PDF soigné, tenant-scoped : page de garde (organisation, référentiel+version, date, demandeur), synthèse exécutive (% de conformité gradué rouge/ambre/vert, barre de progression, répartition par statut) et tableau paginé des contrôles (réf, nom, statut coloré, nombre de preuves, citation source). **Clean Architecture stricte :**
  - `backend/pkg/report/` : renderer **pur** (zéro DB/HTTP, testable isolément) — `RenderCompliancePDF(ComplianceReportData) ([]byte, error)` via `go-pdf/fpdf` (déjà en dépendance). Piège résolu et documenté : `fpdf.SplitText` indexe la table de largeurs par rune sur 256 entrées → **panique** sur tout rune > 255 (tiret cadratin, ligature œ, apostrophe typographique) ; remplacé par un word-wrap maison (`wrapText`) basé sur `GetStringWidth` (octet, cohérent avec le rendu `MultiCell`). Texte français rendu correctement via le translator cp1252. 3 tests (PDF valide, contrôles vides, locale EN).
  - `internal/application/compliance/generate_compliance_report.go` : `GenerateComplianceReportUseCase` assemble les données tenant-scoped (framework global + contrôles/preuves du seul tenant demandeur), tallies de progression alignées sur `GetComplianceProgressUseCase`, résolution best-effort du nom d'org/utilisateur (lookup en échec ⇒ dégradation gracieuse, jamais d'échec du rapport). 4 tests (Success, FrameworkNotFound typé, NoControls, LookupErrorsDegradeGracefully).
  - `domain.ComplianceRepository.CountEvidencesByFramework` : comptage des preuves par contrôle en **une seule requête groupée** (join + group by), scopée par tenant de la preuve **ET** tenant du contrôle (défense en profondeur) ; 1 test repository dédié (isolation tenant + isolation par framework).
  - Handler `GenerateReport` (stream PDF + `Content-Disposition` avec nom de fichier descriptif ex. `compliance-report-iso-27001-2022.pdf`), route sous `protected` avec `compliance:controls:read`, OpenAPI à jour (`generateComplianceReport`).
  - Frontend : bouton « Rapport PDF » dans `/compliance` (hook `useComplianceReport`, `complianceService.downloadReport` — même pattern blob que le download de preuve, lit le nom de fichier depuis `Content-Disposition`), i18n FR/EN (`compliance.report.*`), état `isPending`/toasts succès/erreur.
  - **Preuve live** : login → rapport ISO 27001 (framework à 198 contrôles ISO+BCEAO+ANTIC-CM+COBAC) → 200 `application/pdf`, 11 pages, 27,6 % de conformité, en-tête de tableau répété à chaque saut de page, footer paginé, accents FR (contrôles ANTIC-CM « Loi n°2010/012 art. 33 ») **et** texte EN (contrôles ISO) corrects côte à côte.
- [x] **Passe UX + gouvernance des référentiels (09-10/07/2026, vérifiée live via Chrome CDP)** : 5 bugs UI corrigés (crash page blanche au clic risque, Mitigations en boucle de chargement, Compliance/Risks non scrollables, import de catalogue non intuitif) ; modals qui cachaient leur bouton submit (Risk/Asset/Mitigation) restructurés (`max-h-[90vh]` + footer épinglé) ; **suppression de framework** (`DELETE /compliance/frameworks/{id}`, admin-only, cascade les contrôles du tenant) ; framework ISO nettoyé de ses 105 contrôles non-ISO (198→93) ; terminologie catalog/framework unifiée dans l'UI ; **frameworks rendus tenant-scoped** (étaient globaux — `tenant_id` + index unique par tenant + migration `0030` + backfill ; isolation prouvée live : un nouveau tenant voit 0 framework, l'admin garde ses 4).
- [x] **Board Report mensuel (IA, human-in-the-loop, ton non-technique, montants en FCFA)** — ✅ fait + vérifié live le 10/07/2026 (backend réel, DB réelle, PDF rendu en images et inspecté, flux complet exercé). **Premier client LLM câblé du projet.** Clean Architecture stricte :
  - `backend/pkg/ai/` (nouveau package, anticipé par CLAUDE.md) : interface `Advisor { GenerateBoardNarrative(ctx, BoardPosture) (BoardNarrative, error); Name() string }` + **`TemplateAdvisor`** (déterministe, pur, FR/EN, sans clé — testé : déterminisme, mentions FCFA/org, cas sans risque critique/sans référentiel, formatage FCFA groupé) + **`ClaudeAdvisor`** (SDK officiel `anthropics/anthropic-sdk-go` v1.56, modèle `claude-opus-4-8`, **adaptive thinking** conformément au skill `claude-api`, prompt système « conseiller GRC, public non technique, FCFA, JSON strict », parse JSON tolérant fences + fallback). Factory `NewAdvisor(apiKey, model)` → Claude si `ANTHROPIC_API_KEY`, sinon Template. `FormatFCFA` (groupage par milliers).
  - `internal/application/board/` : `GenerateBoardReportUseCase` agrège la posture **réelle tenant-scoped** — risques par criticité (`CountRisksByCriticality`, méthode concrète sur `GormRiskRepository`, hors interface `RiskRepository` pour ne pas casser les mocks ; `LOWER(criticality)` vu les deux vocabulaires), conformité par référentiel (réutilise `ComplianceRepository`), **exposition FCFA** via `ExposureModel` (valeurs de référence par criticité, ordre de grandeur assumé, documenté). Appelle l'Advisor puis **retombe sur le TemplateAdvisor si erreur** (jamais d'échec ; `generated_by_model` enregistre honnêtement qui a rédigé). Use cases Get/List/Update (brouillon uniquement)/Approve (fige, enregistre approbateur+date)/Delete.
  - `domain.BoardReport` (entité tenant-scoped : snapshot posture gelé + narration éditable + statut draft→approved + provenance + org name) + `BoardReportRepository` + `GormBoardReportRepository` (WHERE id AND tenant_id partout) ; ajouté à `AutoMigrate` (colonnes text[]/jsonb gérées par GORM, pas de migration SQL manuelle).
  - `pkg/report/board_pdf.go` : `RenderBoardPDF(BoardReportData)` **pur**, réutilise les helpers du rapport de conformité (palette, `wrapText`, `MultiCell` sûr) — bandeau de garde + badge statut (BROUILLON ambre / APPROUVÉ vert) + 3 cartes KPI (conformité gradée, risques dont critiques, exposition FCFA) + puces de criticité + tableau conformité par référentiel à barres + sections narratives + recommandations numérotées + footer paginé (bannière « BROUILLON — non diffusable » sur les brouillons). 2 tests de rendu (FR accents+tiret cadratin, EN approuvé sans référentiel).
  - Handler `BoardReportHandler` + routes sous `protected` (`reports:board:read|create|update|approve|delete`) : `POST /reports/board` (génère un brouillon), `GET /reports/board`, `GET /reports/board/{id}`, `PATCH …` (édite le brouillon), `POST …/approve`, `DELETE …`, `GET …/pdf`. OpenAPI à jour (schémas `BoardReport`/`FrameworkSnapshot`/`Generate…`/`Update…Input` + 5 paths).
  - Frontend : `features/reports/BoardReportPage.tsx` + `useBoardReports`/`useBoardReport` (React Query) + `services/boardService.ts` + `types/board.ts` ; item sidebar « Board Report » + route `reports/board`. 3 états UI (skeleton/erreur/vide), génération (choix locale FR/EN + période auto), liste, détail avec narration **éditable** (textarea) tant que brouillon, boutons Enregistrer/Approuver/PDF/Supprimer, barres de conformité, `tsc -b` OK.
  - **Preuve live (10/07/2026)** : login → `POST /reports/board` → brouillon avec agrégation réelle (4 référentiels du tenant : ANTIC/BCEAO/COBAC/ISO, conformité globale 21,1 % = 38/180 applicables, 1 risque moyen → 3 000 000 FCFA, `created_by` renseigné, narration FR non technique) → `PATCH` (synthèse + recommandations éditées, persistées) → `POST …/approve` (status approved, approved_by/at renseignés) → `PATCH` post-approbation **rejeté 400** (figé) → `GET …/pdf` 200 `application/pdf` rendu en PNG et inspecté (organisation « OpenDefender », badges, barres ISO 51 % 38/75, texte édité, bannière brouillon puis footer confidentiel après approbation, accents+œ+tiret cadratin corrects). **Le chemin ClaudeAdvisor n'a pas pu être exercé live faute de clé** (pas d'`ANTHROPIC_API_KEY` en env) — il compile, suit le skill `claude-api`, et le use case retombe sur le template ; à revérifier live dès qu'une clé est fournie.
- **Acceptation M4 : atteinte** — rapport de conformité **et** Board Report générés, relisibles, tenant-scoped, éditables, validables avant diffusion, montants FCFA. LLM optionnel (dégradation gracieuse sans clé).
...
- [ ] Super Admin (pour equipe OpenDefender) · Feature Flags
...
| **Wave 1** | **M1 Compliance engine ✅ fait + vérifié live 08/07/2026** (voir §3) + **M2 ✅ fait 08/07/2026** : ISO 27001 (93 contrôles, vérifié live) **+ BCEAO/UEMOA (35) + ANTIC-CM (25) + COBAC (45)** cités article par article (voir §3) + **M3 Assets ✅ fait le 08/07/2026** (Clean Architecture + snapshots + criticité→Score Engine, voir §3) + **M4 🟡 partie « rapport de conformité en 1 clic » ✅ faite + vérifiée live le 09/07/2026** (PDF soigné tenant-scoped, `pkg/report` pur + use case + endpoint + bouton front, voir §3 M4) ; **reste le Board Report IA/FCFA (❌)** + Incident (🟡, table manquante) + Offline (❌) + 3 écrans parfaits (❌, + nav coherence ajoutée) | 🟡 M1+M2+M3 faits, M4 à moitié (rapport conformité ✅, Board Report ❌) |
