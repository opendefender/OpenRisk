...
| **Wave 1** | **M1 Compliance engine ✅ fait + vérifié live 08/07/2026** (voir §3) + **M2 ISO 27001 ✅ fait + vérifié live 08/07/2026** (93 contrôles importables en un clic, voir §3) + COBAC/BCEAO/ANSSI-CM ❌ (délibérément reporté, textes source manquants) + **M3 Assets ✅ fait le 08/07/2026** (Clean Architecture + snapshots + criticité→Score Engine, voir §3) + Reporting/Board (❌, M4 suivant) + Incident (🟡, table manquante) + Offline (❌) + 3 écrans parfaits (❌, + nav coherence ajoutée) | 🟡 M1+M2(ISO)+M3 faits, priorité = M4 (Reporting/Board) ou COBAC dès textes source disponibles |
