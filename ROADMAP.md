...
| **6. Infrastructure Scanner** (cloud + Agent on-prem) | 🟡 (backend prouvé live) | **Backend complet 14/07** (`internal/scanner/` : interface `Scanner`, types discovery, pipeline `Validate→Scan→Normalize→Deduplicate→StorePreview→Notify` qui **n'écrit jamais** Asset/Risk, preview Redis 48h, dedup, **auto-mitigation par diff**). Scanners cloud AWS/Azure/GCP (validation creds AES-256-GCM + intégration pipeline, **collector SDK = seam nil → preview vide honnête**) ; agent nmap/agent (scope /24). Sous-système Agent : token scopé `scanner` (7j) + registration token (24h) + **HMAC-SHA256** sur push + lock Redis (1 scan/config, 3/tenant) + SSE jobs. 8 endpoints + import/ignore/preview/registration-token, isolation tenant, 30+ tests. **Flux prouvé live de bout en bout** (login→config→scan→register agent→push HMAC→preview dedup+criticité inférée→import DB→mitigation N-1→révocation). | **Enumération SDK cloud réelle** (EC2/S3/SecurityHub, Defender, SCC) = collector à brancher. **Binaire Agent** (Go <15 Mo, nmap/osquery, systemd/service, tray, auto-update) = à écrire. **Frontend** `features/infrastructure` (InfrastructurePage, ScanConfigDrawer, ScanPreviewPage) = à faire. Notif in-app/email sur scan (SSE seul câblé). |
