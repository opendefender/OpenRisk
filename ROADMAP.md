...
- [ ] **Board Report mensuel** (IA, human-in-the-loop, ton non-technique, montants en **FCFA**) — **pas commencé**, seconde moitié de M4. Dépend d'un LLM câblé (aucun client LLM branché aujourd'hui — voir `ai_risk_predictor_service`/`recommendation_service`, stubs sans provider).
