...
### M4 — Reporting officiel + Board Report 🟡 (partie « rapport de conformité en 1 clic » ✅ faite le 09/07/2026, branche `feat/m4-compliance-report-pdf`)
...
- [ ] **Board Report mensuel** (IA, human-in-the-loop, ton non-technique, montants en **FCFA**) — **pas commencé**, seconde moitié de M4. Dépend d'un LLM câblé (aucun client LLM branché aujourd'hui — voir `ai_risk_predictor_service`/`recommendation_service`, stubs sans provider). Plan : `pkg/ai` (interface `Advisor` + `ClaudeAdvisor` si `ANTHROPIC_API_KEY` + fallback `TemplateAdvisor`) → `GenerateBoardReportUseCase` (posture risque/conformité + exposition FCFA + narration) → statut draft/approved → PDF via `pkg/report` → front.
- **Acceptation (partie conformité) : atteinte** — PDF généré, relisible, tenant-scoped, validable avant diffusion. Reste le Board Report.
