...
| **4. Mitigation Workflow** | ✅ | Backend (`application/mitigation`, sous-actions, progress→review) + Kanban frontend (`features/mitigations`). Routes d'écriture corrigées (bug `RequireRole`). | **Auto-complétion par le scanner (4.1) = ❌** car Module 6 absent. Vue Gantt à confirmer. |
...
| **6. Infrastructure Scanner** (cloud + Agent on-prem) | ❌ | Aucun `internal/scanner/`, aucun Agent, aucun endpoint scanner. | Tout : interface Scanner, AWS/Azure/GCP, Agent nmap/osquery, preview Redis, auto-mitigation. Gros chantier. |
