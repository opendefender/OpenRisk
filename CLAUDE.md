...
- **Solide (✅)** : Risk Register, Mitigation, Score Engine, Notifications, Audit logging, i18n FR/EN, **Compliance Frameworks — M1 complet, vérifié live de bout en bout le 08/07/2026** (use cases + handlers + OpenAPI + client généré + frontend + upload de preuve réel + RBAC granulaire, voir `ROADMAP.md` §3 M1). Auth (JWT RS256, Argon2id, MFA, OAuth2/SAML2) et RBAC multi-tenant : code présent et maintenant fonctionnel de bout en bout (login réel testé le 08/07/2026 après correction de 8 bugs, voir `ROADMAP.md` §3 M1) mais **jamais vérifié en profondeur au-delà du login** — traiter chaque sous-flux (MFA, OAuth2/SAML2, refresh token) comme non prouvé tant qu'il n'a pas été testé live.
- **Partiel (🟡)** : Assets, Incident Management (table manquante d'`AutoMigrate`), Custom Fields, Marketplace (structs sans tags `gorm:`, exclues d'`AutoMigrate` le 08/07/2026), PAM Audit Trail, CTI Engine (non câblé, en avance de phase Wave 2), Dashboard/Analytics (**401 systématique sur `/analytics/*` et `/risks` découvert le 08/07/2026, cause distincte non investiguée — voir priorité 5 ci-dessous**).
...
5. Bloquant découvert le 08/07/2026, à corriger avant toute nouvelle démo live du Dashboard : `/api/v1/analytics/*` et `/api/v1/risks` répondent 401 pour une cause distincte non investiguée (pas le même bug que le login, déjà corrigé). L'intercepteur axios global redirige vers `/login` au moindre 401 n'importe où dans l'app, donc ce bug peut éjecter un utilisateur même sur une page qui fonctionne (ex: Compliance).
6. Prochaine étape fonctionnelle : **M2 — contenu réglementaire africain** (ISO 27001 d'abord, puis COBAC/BCEAO/ANSSI-CM) par-dessus le moteur M1 (voir `ROADMAP.md` §3, M2).
7. Hors scope immédiat mais à planifier : ~350 findings lint frontend, 7 fichiers de tests frontend en échec (pré-existants, build), rétrofit du client OpenAPI généré sur Risk/Mitigation (pré-existant, M1), `TestRiskCRUDFlow`/`TestSetupMFA_Success` en échec (pré-existants, découverts pendant M1 — `TestStartAndStop` repasse au vert depuis le 08/07/2026, probablement flaky plutôt que corrigé).
