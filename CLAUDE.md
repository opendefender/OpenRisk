...
7. Prochaine étape : soit **COBAC/BCEAO/ANSSI-CM** dès que de vrais textes source sont disponibles (voir `ROADMAP.md` §3 M2), soit avancer sur **M3/M4** (Assets, Reporting) en attendant.
8. Hors scope immédiat mais à planifier : `CreateRisk` ne renseigne pas `created_by` depuis le contexte réel ; implémenter `/analytics/security-score` et `/analytics/assets/statistics` (routes jamais créées, widgets en repli gracieux) ; ~350 findings lint frontend ; 7 fichiers de tests frontend en échec (pré-existants, build) ; rétrofit du client OpenAPI généré sur Risk/Mitigation (pré-existant, M1) ; `TestRiskCRUDFlow`/`TestSetupMFA_Success` en échec (pré-existants, découverts pendant M1 — `TestStartAndStop`/`TestRateLimit_DifferentIPs` flaky, repassent au vert en isolation).
