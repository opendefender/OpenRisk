...
func TestMarketplaceService_ListConnectors(t testing.T) {
	...
	// List connectors
	connectors, total, err := service.ListConnectors(ctx, nil, "", , )
	require.NoError(t, err)
	assert.Equal(t, int(), total)
	assert.Len(t, connectors, )
}
...
func TestMarketplaceService_SearchConnectors(t testing.T) {
	...
	results, total, err := service.SearchConnectors(ctx, "Splunk", , )
	require.NoError(t, err)
	assert.Greater(t, total, int())
	assert.Greater(t, len(results), )
}
...
func TestMarketplaceService_InstallApp(t testing.T) {
	...
	app, err := service.InstallApp(ctx, connector.ID, "tenant-", "user-", "My Connector", config)
	require.NoError(t, err)
...
func TestMarketplaceService_InstallApp_AlreadyInstalled(t testing.T) {
	...
	_, err = service.InstallApp(ctx, connector.ID, "tenant-", "user-", "My Connector", nil)
	require.NoError(t, err)
...
func TestMarketplaceService_EnableDisableApp(t testing.T) {
	...
	app, err := service.InstallApp(ctx, connector.ID, "tenant-", "user-", "My Connector", nil)
	require.NoError(t, err)

	// Disable app
	err = service.DisableApp(ctx, app.ID, "user-")
	require.NoError(t, err)
	...
	// Enable app
	err = service.EnableApp(ctx, app.ID, "user-")
	require.NoError(t, err)
...
func TestMarketplaceService_UpdateApp(t testing.T) {
	...
	updated, err := service.UpdateApp(ctx, app.ID, "user-", newConfig)
	require.NoError(t, err)
...
func TestMarketplaceService_TriggerSync(t testing.T) {
	...
	// Trigger sync
	err = service.TriggerSync(ctx, app.ID, "user-")
	require.NoError(t, err)
...
func TestMarketplaceService_UninstallApp(t testing.T) {
	...
	// Uninstall app
	err = service.UninstallApp(ctx, app.ID, "user-")
	require.NoError(t, err)
...
func TestMarketplaceService_GetAppLogs(t testing.T) {
	...
	// Get logs
	_, total, err := service.GetAppLogs(ctx, app.ID, "", , )
	require.NoError(t, err)
	assert.Greater(t, total, int())
}
...
func TestMarketplaceService_AddConnectorReview(t testing.T) {
	...
	// Add review
	err = service.AddConnectorReview(ctx, connector.ID, "user-", "John Doe", , "Excellent connector!")
	require.NoError(t, err)

	// Verify review was added
	updated, err := service.GetConnector(ctx, connector.ID)
	require.NoError(t, err)
	assert.Len(t, updated.Reviews, )
	assert.Equal(t, , updated.Reviews[].Rating)
}
