...
type keepRevoker struct{ keep string }

func (k *keepRevoker) RevokeAllExcept(_ context.Context, _ uuid.UUID, keep string) (int64, error) {
	k.keep = keep
	return 1, nil
}
...
func TestChangePasswordHTTP(t *testing.T) {
	...
	app.Post("/auth/password/change", func(c *fiber.Ctx) error {
		if c.Get("X-No-Session") == "" {
			middleware.SetContext(c, &middleware.RequestContext{UserID: user.ID})
		}
		return c.Next()
	}, h.ChangePassword)
	...
	code, body := call(map[string]string{"current_password": oldPw, "new_password": newPw, "locale": "en"}, map[string]string{"X-Refresh-Token": "this-device-refresh-token"})
	if code != http.StatusOK || body["other_sessions_revoked"] != float64(1) {
		t.Fatalf("success: %d %v", code, body)
	}
	if revoker.keep == "" || revoker.keep == "this-device-refresh-token" {
		t.Fatalf("the current session must be kept by its HASH, got %q", revoker.keep)
	}
