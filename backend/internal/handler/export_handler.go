...
func ExportRisksPDF(c *fiber.Ctx) error {
	...
	if err := database.DB.Where("deleted_at IS NULL AND tenant_id = ?", tenantID).Preload("Assets").Find(&risks).Error; err != nil {
	...
	for _, risk := range risks {
		...
		pdf.CellFormat(colWidths[2], 6, fmt.Sprintf("%.2f", risk.Impact), "1", 0, "C", false, 0, "")
		pdf.CellFormat(colWidths[3], 6, fmt.Sprintf("%.2f", risk.Probability), "1", 0, "C", false, 0, "")
