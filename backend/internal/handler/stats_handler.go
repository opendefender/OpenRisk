...
func GetRiskMatrixData(c *fiber.Ctx) error {
	...
	err := database.DB.Table("risks").
		...
		Where("deleted_at IS NULL AND tenant_id = ?", tenantID). // N'inclut pas les risques archivés
...
func GetGlobalRiskTrend(c *fiber.Ctx) error {
	...
	err := database.DB.Table("risks").
		...
		Where("deleted_at IS NULL AND tenant_id = ? AND created_at >= ?", tenantID, time.Now().AddDate(0, 0, -30)).
...
func GetMitigationMetrics(c *fiber.Ctx) error {
	...
	database.DB.Table("mitigations").
		Where("deleted_at IS NULL AND tenant_id = ? AND status = ?", tenantID, "DONE").
		Count(&completed)

	// Compter les mitigations en cours
	database.DB.Table("mitigations").
		Where("deleted_at IS NULL AND tenant_id = ? AND status = ?", tenantID, "IN_PROGRESS").
		Count(&inProgress)

	// Compter les mitigations planifiées
	database.DB.Table("mitigations").
		Where("deleted_at IS NULL AND tenant_id = ? AND status = ?", tenantID, "PLANNED").
		Count(&planned)
