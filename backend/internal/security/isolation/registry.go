...
var decisions = []Decision{
	...
	{"/api/v1/saved-views/{id}", Covered,
		"repository/gorm_saved_view_repository_test TestSavedViewRepo_CrossTenant_IsInvisible proves a foreign id reads back nil and that update and delete affect zero rows; application/savedview TestSavedViews_CrossTenant_IsNotFound_Not403 asserts the update and delete paths answer ErrNotFound for a foreign view with the SAME error a fabricated id produces (a 403 would confirm the row exists, and a view's filter values name that tenant's assets and people). Another user's PERSONAL view inside the same tenant is 404 for the same reason; only a SHARED view answers 403, and only because the caller can already see it"},
	...
	{"/api/v1/organization/logo", Covered,
		"#718: GetOrganizationLogo takes no id and reads orgs.GetByID(ctx, tenantID) with the tenant from the token; handler/organization_profile_e2e_test TestOrganizationBranding_HTTP and membership TestGetOrganizationLogo_CrossTenant assert tenant B gets 404 while tenant A has a logo"},
	{"/api/v1/organization/branding", Covered,
		"#718: GetBranding takes no id and reads the token's own organisation; handler/organization_profile_e2e_test TestOrganizationBranding_HTTP asserts tenant B's branding carries none of tenant A's logo or accent"},
