...
const (
	...
	TokenTypeOAuth  TokenType = "oauth"  // OAuth 2.0 tokens
)
...
type APIToken struct {
	...
	TokenHash    string                 `json:"-"`                                  // SHA256 hash of actual token (for storage)
	TokenPrefix  string                 `json:"token_prefix"`                       // First 8 chars of token (for display)
