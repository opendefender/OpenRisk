...
func GetDashboardStats(c *fiber.Ctx) error {
	...
	for _, r := range risks {
		...
		severity := "LOW"
		if r.Score >= 20 { severity = "CRITICAL" } else 
		if r.Score >= 15 { severity = "HIGH" } else
		if r.Score >= 10 { severity = "MEDIUM" }
