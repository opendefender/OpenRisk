...
func GetRiskMatrixData(c fiber.Ctx) error {
	...
	// Requête groupe pour compter les risques par paire (Impact, Probability)
...
func GetGlobalRiskTrend(c fiber.Ctx) error {
	...
	// Simulation du score de scurit (où  est parfait)
	currentScore := 
...
func GetRiskDistribution(c fiber.Ctx) error {
	...
	// Requête groupe pour compter les risques par niveau
	err := database.DB.Table("risks").
...
func GetTopVulnerabilities(c fiber.Ctx) error {
	...
	if limit >  {
		limit =  // Limiter pour viter les requêtes trop lourdes
	}
