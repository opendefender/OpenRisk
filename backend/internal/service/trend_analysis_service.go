...
func (s *TrendAnalysisService) AnalyzeTrend(tenantID, metricType string, dataPoints []float64, timestamps []time.Time, timeRange int) *domain.TrendAnalysis {
	if len(dataPoints) < 2 {
		return nil
	}
...
func (s *TrendAnalysisService) GenerateForecast(tenantID, metricType string, dataPoints []float64, timestamps []time.Time, forecastDays int) *domain.TrendForecast {
	if len(dataPoints) < 3 {
		return nil
	}

	forecast := &domain.TrendForecast{
		ID:              generateID(),
		TenantID:        tenantID,
		MetricType:      metricType,
...
func (s *TrendAnalysisService) GenerateRecommendations(tenantID string, analysis *domain.TrendAnalysis, forecast *domain.TrendForecast) []domain.TrendRecommendation {
	var recommendations []domain.TrendRecommendation

	// Recommendation 1: Anomaly alert
	if analysis.IsAnomalous {
		rec := domain.TrendRecommendation{
			ID:                generateID(),
			TenantID:          tenantID,
...
func (s *TrendAnalysisService) FilterTrends(trends []domain.TrendAnalysis, filter domain.TrendFilter) []domain.TrendAnalysis {
	var filtered []domain.TrendAnalysis
...
func (s *TrendAnalysisService) ExportTrendData(analysis *domain.TrendAnalysis, forecast *domain.TrendForecast, recommendations []domain.TrendRecommendation) *domain.TrendExportData {
	return &domain.TrendExportData{
		MetricType:      analysis.MetricType,
		Analysis:        *analysis,
		Forecast:        *forecast,
...
func (s *TrendAnalysisService) selectBestModel(data []float64, forecastDays int) []domain.PredictedValue {
	...
	predictions := make([]domain.PredictedValue, forecastDays)
	startIdx := float64(len(data))

	for i := 0; i < forecastDays; i++ {
		x := startIdx + float64(i)
		predicted := intercept + slope*x
		predictions[i] = domain.PredictedValue{
...
func (s *TrendAnalysisService) validateForecast(actual []float64, predicted []domain.PredictedValue) float64 {
...
func (s *TrendAnalysisService) calculateRMSE(actual []float64, predicted []domain.PredictedValue) float64 {
	minLen := len(actual)
...
func (s *TrendAnalysisService) calculateMAPE(actual []float64, predicted []domain.PredictedValue) float64 {
	minLen := len(actual)
	if len(predicted) < minLen {
		minLen = len(predicted)
	}
