...
> **Statut : RATIFIÉE par le fondateur le 2026-07-24 — IMPLÉMENTÉE** (`frontend/src/shared/navModel.ts`),
> régressée par le merge `91df861`, **rétablie par #270 le 2026-09-16**. La sidebar est
> groupée en 5 intentions + utilitaire ; ce qui reste à faire est listé au §6 « État ».
...
- ✅ Placeholders **Leaderboard/Simulations retirés** de la sidebar (routes conservées) ; **Infrastructure** et **Asset Universe** dé-« soon ».
...
### Régression puis rétablissement (#270, 2026-09-16)
Le merge `91df861` (`Merge branch 'master' into feat/ia-nav-ui-elevation`) a mal résolu `navModel.ts` : il a laissé un second groupe `g_pilot` portant les anciennes entrées « cœur » (Risques, Vulnérabilités, Mitigations, Incidents, Automatisation) et a déplacé le vrai contenu de Piloter sous une clé `g_monitor` (« Surveiller »). Résultat à l'écran jusqu'au 2026-09-16 : **7 groupes et 5 entrées affichées deux fois**. Ce document avait par ailleurs été supprimé par `f183410` (2026-08-20) alors que `docs/UX_CHARTER.md` (UX-16) et `ROADMAP.md` le citaient encore.

#270 supprime le groupe en double, rend `g_pilot` à Piloter, retire la clé orpheline `g_monitor`, restaure ce document, et ajoute `frontend/src/shared/__tests__/navModel.test.ts`, qui échoue si l'ordre des intentions change, si le premier niveau dépasse 7 entrées, ou si une entrée apparaît dans deux groupes.

### Ce que rend la sidebar aujourd'hui (source : `navModel.ts`)
Premier niveau : **Tableau de bord** (épinglé) + **6 groupes** = **7 entrées** (UX-16 : ≤ 7 ✅). Les entrées sont filtrées par permission ; un groupe vide pour le rôle n'est pas affiché.

| Groupe | Entrées (route) |
|---|---|
| **Piloter** | Tableau de bord `/` (épinglé) · Centre d'actions `/action-center` · Tableau exécutif `/?view=executive` · Quantification financière `/analytics/financial` · Activité `/activity` |
| **Identifier** | Inventaire `/assets` · Topologie `/assets/topology` · Fournisseurs `/vendors` · Vulnérabilités `/vulnerabilities` · Threat Intel `/threat-map` · Infrastructure `/infrastructure` |
| **Évaluer** | Registre des risques `/risks` |
| **Traiter** | Mitigations `/risks/mitigations` · Incidents `/incidents` · Automatisation `/automation` |
| **Prouver** | Conformité `/compliance` · Preuves `/compliance/evidence` · Rapports `/reports` · IA Advisor `/recommendations` · Risques émergents `/ai/emerging-risks` · Gouvernance `/governance` |
| **Admin** (utilitaire) | Rôles & accès `/settings/members` (admin) · Paramètres `/settings` |

Entrées ajoutées après la ratification et rangées selon §5 (UX-31) : Centre d'actions et Activité (Piloter, #433 / W1-02), Preuves (Prouver), Fournisseurs (Identifier, #673 — un fournisseur est un actif, ADR 0004 D1).

### Pas encore fait (reste honnête)
- **Fusion Dashboard + Exécutif + Financier en une vue par rôle** (§2, intention 0) : ce sont toujours trois entrées distinctes de Piloter.
- **IA en une seule entrée** (§3) : IA Advisor et Risques émergents restent deux entrées, conformément à l'arbitrage (b) du 2026-07-24.
- **Rôles & accès fusionné dans Paramètres** (§3) : une entrée « Rôles & accès » subsiste dans le groupe utilitaire.
- **Utilitaire en pied de sidebar** (§2) : le groupe Admin est rendu comme dernier groupe de la liste, pas en pied.
