 OpenRisk ‚Äî Roadmap & TODO (v√rifi√ le d√p√t)
...
L√gende:
-  = impl√ment√ / livr√
-  = partiellement livr√ / PoC / √† stabiliser
- ‚¨ú = non d√marr√ / √† planifier

PRINCIPES : garder la liste focalis√e (‚Äì priorit√s actives), commencer les features critiques par un PoC, ajouter crit√res d'acceptation pour chaque priorit√.

=== R√âCAPITULATIF RAPIDE (v√rifi√) ===
...
- ‚¨ú RBAC & multi-tenant: mentionn√ en docs mais non impl√ment√.
- ‚¨ú Helm / ks charts: docs/README mention Helm, pas de chart produit.
- ‚¨ú Marketplace, advanced connectors (Splunk, Elastic, AWS Security Hub): listed as PoC priorities, non impl√ment√s.

---

 Priorit√s Imm√diates (MVP ‚Üí  jours)
 ...
 - Actions:
   ...
   - ‚¨ú Lier events: √mettre webhook risk.created et mitigation.updated depuis handlers (PoC) (- jours).
 - Crit√res d'acceptation:
   ...
   - ‚¨ú Webhook document√ et PoC impl√ment√.

) API-First & OpenAPI completion (status:  DONE)
 - Actions:
   -  √âtendre docs/openapi.yaml : couverture compl√te des  endpoints (Health, Auth, Risks CRUD, Mitigations CRUD, Sub-actions, Assets, Statistics, Export, Gamification).
 ...
 - Statut: Livr√ le --. Pr√™t pour tooling (swagger-ui, redoc, code generation).

) Tests & CI (status:  % DONE ‚Äî pipeline & local test setup complete)
 - Actions:
 ...
 - Livr√:
   -  .github/workflows/ci.yml : lint ‚Üí test ‚Üí build ‚Üí push pipeline.
...
) Sync Engine hardening (status:  DONE ‚Äî  jours)
 - Actions:
...
- Priorit√s PoC ‚Üí Production : TheHive (done/PoC) ‚Üí OpenCTI (config existante) ‚Üí Cortex (playbooks) ‚Üí Splunk/Elastic.
- Ajouter EventBus / Webhooks et un broker l√ger (NATS / Redis streams) pour d√coupler int√grations.

Crit√res d'acceptation pour un connecteur pr√™t-prod:
...
- RBAC & Multi-tenant : planifier PoC (phase : RBAC simple bas√ sur roles, phase : tenant isolation via tenant_id sur tables). Non impl√ment√ ‚Üí Priorit√ Q.
- Hardening: dependency SCA, security scans, CSP, rate limiting (d√j√† headers Helmet middleware pr√sent).

---

 UX & Design System

- Cr√er OpenDefender Design System (tokens Tailwind + Storybook) ‚Äî planifier sprint d√di√.
- Prioriser onboarding flows et dashboard widgets (drag & drop futur).

---

 Roadmap courte ( livrables concrets)
. Mitigation sub-actions (API + UI) ‚Äî  jours 
. OpenAPI full coverage + API_REFERENCE ‚Äî  jours 
. CI (GitHub Actions) + integration tests ‚Äî  jours 
. Sync Engine hardening (idempotency, retries, metrics) ‚Äî  jours 
. RBAC PoC (role-based, auth middleware) ‚Äî  jours ‚¨ú
...
- GitHub Actions: .github/workflows/ci.yml (+ lines)
  - Stages: lint ‚Üí test ‚Üí build ‚Üí docker push
  - Backend: golangci-lint, go test, coverage
  - Frontend: ESLint, tsc, npm test
- Docker: Multi-stage build (backend Go ‚Üí Node frontend ‚Üí Alpine runtime)
...
- TheHive adapter enhancements
  ...
  - Severity mapping (- ‚Üí domain values)
  - Error handling with fallback to mock data
- Test suite:  tests ( sync engine +  adapter) ‚Äî all passing
...
- Navigation: Updated App.tsx router, Login ‚Üí Register link
- Status: Production-ready
...
-  Advanced permission matrices (resource-level access control) ‚Äî Completed Session 
-  API token management for service accounts ‚Äî Completed Session 
- SAML/OAuth integration (single sign-on)
...
- ActionRead/Create/Update/Delete ‚Üí PermissionRead/Create/Update/Delete
- ResourceRisk/Mitigation/User/AuditLog ‚Üí PermissionResourceRisk/Mitigation/User/AuditLog  
- ScopeOwn/Team/Any ‚Üí PermissionScopeOwn/Team/Any
...
. Type Collisions ( file) 
   - Renamed lucide icon import: Users ‚Üí UsersIcon in Users.tsx
...
- Fixed import paths from openrisk ‚Üí github.com/opendefender/openrisk
...
-  Validation avanc√e (regex, formats, d√pendances entre champs) ‚Äî PoC in form components, needs framework templates
-  Custom Fields v (users peuvent ajouter champs texte/choix/num√rique) ‚Äî DONE Phase 
- ‚¨ú Fields templates par framework (ISO, NIST‚Ä ‚Üí auto-population) ‚Äî BLOCKED: needs security expertise/data
-  Bulk actions (update, delete, assign mitigations, tags) ‚Äî DONE Phase 
-  Risk timeline (tous les √v√nements : cr√ation, update, mitigation, sync) ‚Äî DONE Phase 

UX & Design System
- ‚¨ú Cr√er l'OpenDefender UI Kit (pens√ pour toute la suite) ‚Äî BLOCKED: needs design team decision
- ‚¨ú Cr√er un composant "DataTable++" maison (filtre multiples, tri, tags, search instant) ‚Äî Feasible: extend current table
- ‚¨ú Heatmap dynamique (drag, hover, zoom) ‚Äî Feasible: use Recharts or D.js
- ‚¨ú Dashboard widgets drag & drop type Airtable / Linear ‚Äî BLOCKED: needs UI Kit
-  Onboarding "guided steps" ‚Äî PoC: components exist (CreateRiskModal, MitigationEditModal, sync engine, Analytics), needs Shepherd.js wrapper

Mitigations & Plans d'Actions
- ‚¨ú Dependencies entre mitigations (bloqueur/conditionnel) ‚Äî Feasible: schema design only
- ‚¨ú Notifications internes (rappels, deadlines) ‚Äî Feasible: use cron jobs or event bus
- ‚¨ú Assignation multi-utilisateur ‚Äî Feasible: extend current handler
- ‚¨ú Templates de plans (ISO, CIS, NIST‚Ä) ‚Äî BLOCKED: needs domain expertise/security best practices DB
- ‚¨ú Vue Gantt / Timeline des actions ‚Äî Feasible: use react-gantt-chart

S√curit√ : RBAC & Multi-Tenant version compl√te
-  RBAC complet avec granularit√ par ressource ‚Äî DONE Phase /
  (risk:update:own, risk:update:any, mitigation:view:team‚Ä)
   Permission domain model with wildcards
   Permission middleware for routes
   Integrated with risk endpoints
- ‚¨ú Isoler tenant_id partout (DB + cache + logs) ‚Äî BLOCKED: needs business model decision (SaaS vs self-hosted)
-  Audit logs (table & API, export JSON) ‚Äî DONE Phase 
   Audit service with full tracking
   AuditLog API endpoints
   Frontend AuditLogs page
- ‚¨ú Rate limiting + throttling (global & tenant) ‚Äî Feasible: use middleware + Redis

Rapports & Export
- ‚¨ú PDF export (√l√gant, brand√ OpenDefender) ‚Äî Feasible: use pdfkit or puppeteer
- ‚¨ú HTML interactive export (offline) ‚Äî Feasible: static HTML generation
-  JSON export + API (interop√rabilit√) ‚Äî DONE Phase : Analytics export endpoint
- ‚¨ú G√n√ration auto de "Rapport de risques" pour audit ‚Äî Feasible: depends on PDF export

Int√grations & Connecteurs
- ‚¨ú OpenCTI (risks ‚Üî threats syncing) ‚Äî BLOCKED: needs OpenCTI instance + API access
- ‚¨ú Cortex (actions automatis√es) ‚Äî BLOCKED: needs Cortex instance + analyzers
- ‚¨ú Elastic & Splunk (logs ‚Üí risk triggers) ‚Äî BLOCKED: needs running clusters
- ‚¨ú SIEM OpenWatch (int√gration native OpenDefender) ‚Äî BLOCKED: ecosystem decision
- ‚¨ú AWS Security Hub (import findings) ‚Äî BLOCKED: needs AWS account
- ‚¨ú Azure Security Center (idem) ‚Äî BLOCKED: needs Azure account
- ‚¨ú Google SCC ‚Äî BLOCKED: needs GCP account
- ‚¨ú EventBus (Redis Streams ou NATS) ‚Äî BLOCKED: needs architecture decision (Redis vs NATS vs Kafka)

Module Assets
- ‚¨ú Inventory associ√ aux risques ‚Äî Feasible: extend risk-asset relationship
- ‚¨ú Lien direct avec OpenAsset ‚Äî BLOCKED: ecosystem alignment decision
- ‚¨ú Impact based on asset business value ‚Äî BLOCKED: needs OpenAsset integration
- ‚¨ú Auto-risks depuis assets critiques ‚Äî BLOCKED: needs OpenAsset + asset data

IA Intelligence Layer
- ‚¨ú D√duplication AI : Compare risques similaires et propose fusion ou suggestion. ‚Äî BLOCKED: needs LLM provider decision
- ‚¨ú Priorisation intelligente : Analyse probabilit√ √ó impact √ó criticit√ des assets √ó tendances. ‚Äî BLOCKED: needs LLM + prioritization formula
- ‚¨ú G√n√ration Mitigations : Propose automatiquement : contr√les √† appliquer, actions correctives, sous-actions, estimation du co√ªt & effort ‚Äî BLOCKED: needs LLM + security best practices DB
- ‚¨ú Detection automation : Construit des risques automatiquement depuis logs / SIEM. ‚Äî BLOCKED: needs SIEM integration + event bus

Installer universel & Ops
-  Helm chart complet ‚Äî DONE Phase :  files (Chart.yaml, values.yaml,  Ks manifests,  env configs)
-  Installer deploy.sh ‚Äî DONE Phase : + lines with pr√-checks, rollback, post-install tests
-  Observability √† % :
       Prometheus metrics ready in Helm chart
       Grafana dashboards included
      ‚¨ú Distributed tracing (OpenTelemetry) ‚Äî Feasible future enhancement
...
- ‚¨ú Normaliser : ‚Äî BLOCKED: ecosystem alignment decision
    ‚¨ú Identifiants d'assets ‚Äî needs OpenAsset coordination
    ‚¨ú Score mod√les ‚Äî needs ecosystem decision
    ‚¨ú Events ‚Äî needs EventBus + other products alignment
    ‚¨ú Permissions ‚Äî needs ecosystem IAM decision
    ‚¨ú UI Kit ‚Äî needs design system decision
- ‚¨ú SSO / IAM commun (Keycloak, Auth, ou maison) ‚Äî BLOCKED: needs business decision on centralized IAM

Community & Adoption
- ‚¨ú Roadmap publique (GitHub Projects) ‚Äî Feasible: GitHub Projects setup
- ‚¨ú Demo en live (Vercel / Render / Fly.io) ‚Äî BLOCKED: needs hosting platform + domain decision
- ‚¨ú Templates d'issues (bug, feature request) ‚Äî Feasible: GitHub issue templates
- ‚¨ú Branding + Site Web OpenRisk ‚Äî BLOCKED: needs marketing/design
- ‚¨ú Post Reddit + LinkedIn + HackerNews ‚Äî Feasible: marketing effort
- ‚¨ú Onboarding vid√o (tu peux le faire une fois) ‚Äî BLOCKED: needs video production resource
---
...
‚úì  columns in api_tokens table
‚úì  indexes created (primary key + unique token_hash +  custom)
‚úì Foreign key constraints on user_id and created_by_id
‚úì JSONB fields for permissions, scopes, ip_whitelist, metadata
‚úì Automatic updated_at timestamp trigger
‚úì All  migrations marked successfully in schema_migrations
...
Test Results:
- Backend compilation: go build -o server ./cmd/server ‚Äî SUCCESS
- Frontend build: npm run build ‚Äî SUCCESS ( KB gzip)
...
- GET /risks ‚Üí risk:read permission
- GET /risks/:id ‚Üí risk:read permission  
- POST /risks ‚Üí risk:create permission
- PATCH /risks/:id ‚Üí risk:update permission
- DELETE /risks/:id ‚Üí risk:delete permission
...
- Display Information:
  ...
  - Expiration tracking with visual warnings (red if expired, amber if ‚â§ days)
...
Remaining:
. ‚è≥ API Marketplace Framework (%)
. ‚è≥ Performance Optimization & Load Testing (%)
. ‚è≥ Mobile App MVP (%)
...
- üü° Phase : Kubernetes & Advanced Analytics (%)
...
|  | Ks & Analytics | üü° IN PROGRESS | % | Ks , Analytics , Marketplace ‚¨ú |
